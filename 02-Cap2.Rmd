...
linear ou logística, modelos log-lineares, etc.). Vários exemplos discutidos ao longo dos Capítulos \@ref(modreg), \@ref(testqualajust) e \@ref(testetab2) ilustram situações deste tipo. No capítulo nove o foco vai ser a estimação não paramétrica da forma da função $f(y;\theta )$. 
...
dessa abordagem é apresentada no Tabela \@ref(tab:modelamo), e uma representação
 gráfica resumida na Figura \@ref(fig:modamo).

```{r, modamo,fig.cap="Amostragem Probabilística",echo=FALSE}
knitr::include_graphics("Figuras/figura22borda.pdf")
```
...
```{r, modsup,fig.cap="Modelagem de Superpopulação", echo=FALSE}
knitr::include_graphics("Figuras/figura23borda.pdf")
```
