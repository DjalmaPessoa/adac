...
conforme já ilustrado no Exemplo \ref{exebin} do Capítulo \@ref(epa). Isto
...
No Exemplo \ref{exebin} do Capítulo \@ref(epa) a estatística de teste $Z_{bin},$ que foi utilizada para comparar com um valor hipotético
...
Calculamos a matriz $\widehat{\mathbf{V}}_{p}$ pela aplicação do método de linearização de Taylor descrito na Seção \ref(#taylor) através da fórmula \@ref(eq:qual22) obtendo
