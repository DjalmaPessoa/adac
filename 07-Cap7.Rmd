...
As proporções populacionais desconhecidas nas categorias são $p_{j}=N_{j}/N$, onde $N$ é o tamanho total da populaç%
ão de empregados e $N_{j}$ é o número de elementos da população na categoria 
...
que $X_{P}^{2}(d)=Z_{bin}^{2}/d= 3,162^{2}/20=0,5$ ou seja, $%
Z_{p}^{2}=X_{P}^{2}(d)$ conforme esperado. Os valores da estatística do
...
es\-ta\-tís\-ti\-ca de bondade de ajuste $X_{N}^{2}$ de Neyman. No caso
