# Estimação Baseada no Plano Amostral {#planamo}
...
(alternativamente, de desvios padrões ou mesmo de coeficientes de varia
ção) é requerida para que intervalos de confiança possam ser
...
especializados em estimação de variâncias de aleatorização (veja discussão no Capítulo 10).
...
\right) ^{2}  (\#eq:estpa27)
\end{equation}
onde $\widehat{\theta }_{J}=\frac{1}{G}\sum_{g=1}^{G}\widehat{\theta }%
_{\left( g\right) }$ é um estimador pontual \emph{jackknife} para $%
\theta ,$ alternativo ao estimador da amostra inteira $\hat{\theta}$.


**Observação:** A descrição do método \emph{jackknife }aqui apresentada não cobre o caso de planos amostrais estratificados, que é mais complexo.
Para detalhes sobre este caso, consulte \citeonline[p. 174]{W85}.
 
**Observação:** O estimador $\widehat{V}_{J2}\left( \widehat{\theta }\right)$ é maisconservador que o estimador $\widehat{V}_{J1}\left( \widehat{\theta }\right)$.
...
Ilustração da linearização de Taylor:

```{r}
tot.est<-svytotal(~ANALF1.SE+FAIXA1.SE,ppv.des)
# Estimativa de cada total
y1hat<-coef(tot.est)[1]
y2hat<-coef(tot.est)[2]
# Estimativa da matriz de covariâncias
Vcov.Y1.Y2<-vcov(tot.est)
```
Calcular por meio da fórmula 3.22 do livro:?

Estimação do se da razão obtida diretamente pela função `svyratio`da library survey

```{r}
ANALF1.SE_FAIXA1.SE_RAZ<-svyratio(~ANALF1.SE,~FAIXA1.SE,ppv.des)
coef(ANALF1.SE_FAIXA1.SE_RAZ)
SE(ANALF1.SE_FAIXA1.SE_RAZ)
```

Material da Seção 3.5:

Jackknife:

```{r}
ppv.des.jkn<-as.svrepdesign(ppv.des,type="JKn")
ANALF1.SE_FAIXA1.SE_RAZ_jkn<-svyratio(~ANALF1.SE,~FAIXA1.SE, ppv.des.jkn)
coef(ANALF1.SE_FAIXA1.SE_RAZ_jkn)
SE(ANALF1.SE_FAIXA1.SE_RAZ_jkn)
```
...
```{r}
ppv.des.jkn<-as.svrepdesign(ppv.des,type="bootstrap")
ANALF1.SE_FAIXA1.SE_RAZ_jkn<-svyratio(~ANALF1.SE,~FAIXA1.SE, ppv.des.jkn)
coef(ANALF1.SE_FAIXA1.SE_RAZ_jkn)
SE(ANALF1.SE_FAIXA1.SE_RAZ_jkn)
```
