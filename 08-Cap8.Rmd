...
válida. Por exemplo, testes definidos em tabelas de freqüências
...
p_{21},p_{22}\right)$. O processo de contagem que gera estas multinomias
...
as freqüências nas celas da tabela são estimadas, levando em
...
padronizado de $\hat{N}_{lc}$, de modo que $\sum\limits_{l=1}^{L}\sum
\limits_{c=1}^{C}\hat{n}_{lc}=n$. Sejam, agora, os estimadores das proporções
...
\ref{Analisec} do Capítulo 5. Se calcularmos as estatísticas 
$X_{P}^{2}\left( I\right)$ e $X_{P}^{2}\left( H\right)$ a partir dos
valores dos $\hat{N}_{lc}$ fornecidos, com a estimativa do tamanho da popula
ção $\hat{N}$ no lugar de $n$, os resultados assintóticos
...
$\hat{n}_{lc}$ anteriormente definidos, que correspondem aos $\hat{N}
_{lc}$ padronizados para totalizar $n$.
...
I\right)$, se baseia no EPA médio das estimativas das proporç
ões nas celas $\hat{p}_{lc}=\hat{n}_{lc}/n$, enquanto que para a
...
de proporções da $l$-ésima região, sem incluir a proporç
ão referente à última categoria ($p_{lC}$), $l=1,2.$ A
...
estimadores ponderados das freqüências nas celas e nas marginais de
...
No caso, a estatística de Wald $X_{W}^{2}\left( H\right)$ tem distribui
ção assintótica qui-quadrado com $\left( 2-1\right) \times
...
\end{equation}
que tem distribuição de referência \emph{F} com $\left(
C-1\right)$ e $f$ graus de liberdade.
...
covariância baseia-se nas hipóteses relativas à distribuiç
ão multinomial, apropriadas para a amostragem aleatória simples. A
...
\]
onde a soma dos quadrados dos autovalores pode ser obtida a partir do tra\c{c
...
L\times C$, onde os dados são extraídos de uma única populaç
ão, sem fixar marginais. Consideremos a Tabela \ref{tab83} com as propor
ções nas celas a nível da população, onde agora
...
estimativa consistente de $\mathbf{f}$ é denotado por $\mathbf{\hat{f}}
=\left( \hat{f}_{11},\ldots ,\hat{f}_{1\;C-1},\ldots ,\hat{f}
...
vetores $\mathbf{f}$ e $\mathbf{\hat{f}}$, pois a soma das proporç
ões nas celas da tabela é igual a 1.
...
\mathbf{p}$, isto é, 
$\mathbf{f=g}\left( \mathbf{p}\right) \mathbf{=g}\left( p_{11},\ldots ,p_{1\;C-1},\ldots ,p_{L-1\;1},\ldots
,p_{L-1\;C-1}\right)$. Assim, a matriz de covariância de 
...
em \@ref(eq:Tab9) e \@ref(eq:Tab10), com $\left( L-1\right) \left( C-1\right)$ no
...
multinomial de $\mathbf{\hat{p}}$ sob a hipótese nula, $\mathbf{\hat{p}}
_{0}$ é o vetor com componentes $\hat{p}_{l+}$ $\hat{p}_{+c}$, e $
...
plano amostral estimados, $\hat{d}_{lc}$, das estimativas das proporç
ões nas celas $\hat{p}_{lc}$. O ajuste mais simples é feito
...
Estimamos os efeitos do plano amostral por $\hat{d}_{lc}=\hat{V}_{p}\left( 
\hat{p}_{lc}\right) /$\linebreak $\left( \hat{p}_{lc}\left( 1-\hat{p}
_{lc}\right) /n\right)$, onde $\hat{V}_{p}\left( \hat{p}_{lc}\right)$
...
ão dos autovalores desconhecidos de $\mathbf{\Delta ,}$ $\delta _{k}$, $
...
Em situações instáveis, pode ser necessário fazer uma corre
ção F ao ajuste de primeira ordem de Rao-Scott \@ref(eq:Tab15). A
...
freqüências nas celas para a amostra pesquisada.
...
```{r, frqamost, results='asis'}
source("~\\GitHub\\adac\\data\\pnadrj90.R")
freq.sx.re <-table(pnadrj90$sx, pnadrj90$re , dnn= c("Sexo", "Renda"))
print(xtable(freq.sx.re ,caption= "Frequências Amostrais por celas na PNADRJ90"), type="html" )
```
...
amostral utilizado na pesquisa (PNAD 90), conforme descrito no Exemplo \ref
{pnad90} do Capítulo 6.

Vamos considerar o teste de homogeneidade entre as variáveis Sexo e Renda
\textbf{\ }e calcular o efeito de plano amostral médio das estimativas
...
10.000 $), e do efeito de plano amostral da estimativa de proporção
na linha.
...
reposição, a estimativa da variância do estimador de proporção de linha na cela é: $0,111\left( 1-0,111\right) /4.276$. A
...
Sexo e Renda\textbf{\ }e calcular o efeito de plano amostral médio das
estimativas das proporções nas celas da tabela. A Tabela \ref
{tab86} contém, em cada cela, as estimativas: da proporção na
cela, do desvio-padrão da estimativa da proporção na cela ($
\times 10.000$), e do efeito de plano amostral da estimativa de proporç
ão na cela.
...
$\left(0,0038343\right) ^{2}$. Sob amostragem aleatória simples com reposiç%
ão, a estimativa da variância do estimador de proporção na
...
corrigir a estatística $X_{P}^{2}\left( I\right)$ é $\hat{d}_{.}=1,640,$ calculada tomando a média dos EPAs das celas
...
$X_{P}^{2}\left( I\right) =X_{P}^{2}\left( H\right) =227,025$, com distribui
ção de referência $\chi ^{2}\left( 2\right)$, resultado que
