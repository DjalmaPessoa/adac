...
para o teste de independência, tem a forma da expressão (\#eq:Tab8), com $\mathbf{\hat{f}}$ no lugar de 
...
Considerando os dados do Exemplo \ref{ex:pnad90} do Capítulo \@ref(modreg), vamos
...
das proporções nas celas da tabela. A Tabela \@ref{tab:proplinha} contém,
...
```{r, proplinha, echo=FALSE, eval=FALSE}
library(survey)
source("~\\GitHub\\adac\\data\\pnadrj90.R")
...
A partir da Tabela \@ref(tab:propcelas), examinando as estimativas das proporç
