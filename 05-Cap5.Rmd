...
A Tabela \@ref(tab51) revela como variam essas probabilidades de seleção entre as 
...
$w_{i}^{R}=\pi_{i}^{-1}X\,/\,\widehat{X}_{\pi }$ (veja \@ref(eq:modpar19), onde $X$ é o total
...
ajustes da teoria aqui exposta (veja \citeonline[cap. 6]{Silva}).
...
\mathbf{\theta }_{U}$ será geralmente adequado também para $\mathbf{
\theta }$.
...
```{definition}
O estimador de Máxima Pseudo-Verossimilhança (MPV) \linebreak $
\mathbf{\hat{\theta}}_{MPV}$ de $\mathbf{\theta }_{U}$ (e consequentemente
de $\mathbf{\theta }$) será a solução das equações de
...
condições sempre que $\pi _{i}>0$ e $\pi _{ij}>0\quad \forall i,j\in
U$ e algumas condições adicionais de regularidade são
...
\mathbf{\hat{\theta}}_{MPV}$ é consistente para a QDPC $\mathbf{\theta}
_{U}$, a inferência baseada no procedimento MPV segue válida para
