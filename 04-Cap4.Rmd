...
```{example}
\label{epakish}Efeitos de plano amostral de Kish para estimadores de totais
...
$\pi$-ponderado. Os resultados referentes à Região Metropolitana do Rio de Janeiro para algumas variáveis são apresentados na Tabela \ref
{tab41} a título de ilustração. Note que a população
...
```{example}
\label{deffuni}Efeitos de plano amostral para estimação de
...
(correlação intraclasse, veja \citeonline[cap. 2]{SilvaMou} e \citeonline{haggard}.
...
![Fig 2.1: Valores de EPA e EPA de Kish para conglomeração](Figuras/figura41borda.pdf)
...
o Exemplo \ref{deffuni} de estimação de médias com amostragem
...
```{example}
\label{deffmult}Efeitos Multivariados do Plano Amostral para as médias
...
{deffuni} e calcular a matriz $\mathbf{EMPA}\left( \mathbf{\hat{\theta},V}_{0}\right)$, onde $\mathbf{\hat{\theta}=}\left( \overline{SAL}_{w},\overline{REC}_{w}\right) ^{\prime }$. Neste exemplo, os dados populacionais
...
mesma notação do Exemplo \ref{deffuni}, temos que 
\[
...
Exemplo \ref{deffuni} e coincidem, respectivamente, com os valores usados
...
Exemplo \ref{deffuni}. Para isto, foram utilizadas outras $500$ amostras de
tamanho $60$ segundo o plano amostral descrito no Exemplo \ref{deffuni}.
...
diferença entre estes $p$valores. Consideremos a região
...
```{r, eval=FALSE}
mat.res<-matrix(NA,500,4)
popul$salario<-popul$salario/1000
library(sampling)
#Geracão dos dados
for(i in 1:500){
s<-strata(popul, "estrato",c(30,30),method= "srswor")
dados<-getdata(popul,s)
med_y<-mean(dados$salario)
var_AAS<-(1-f)*var(dados$salario)/n
desenho<-svydesign(~1,strata=~estrato,data=dados,fpc=~Prob)
med_yw<-svymean(~salario,desenho)
mat.res[i,]<-c(med_y,coef(med_yw),var_AAS,SE(med_yw)^2)
}
colMeans(mat.res)
```
