...
Depois de baixar os dados, são criados os seuintes arquivos no diretório
\texttt{C:/adac/PNS} :
...
\iffalse {Observação } \fi Nos arquivos acima, além de as variáveis
...
\BeginKnitrBlock{remark}
\iffalse {Observação } \fi No data frame pes\_sel os códigos das
variáveis foram transformados para letras minúsculas.
...
é feito por meio da função \texttt{svydesign()} da library `survey.
...
O objeto de desenho \texttt{pes\_sel\_des} deve ser modificado para
incorporar informações de totais disponíveis a partir do Censo
Demográfico. Para isso, são definidos \texttt{post-pop}, definido a
seguir.
...
incorpora-se no objeto de desenho \texttt{pes\_sel\_des} as informações
contidas no data frame \texttt{post\_pop}.
...
As estimativas e os limites do intervalo de confiança aproximado com
95\% de nível de confiança podem ser obtidos por meio da função:
