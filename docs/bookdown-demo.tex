...
\protect\hypertarget{ex:unnamed-chunk-3}{}{\label{ex:unnamed-chunk-3}}Distribuição
...
\midrule
Nordeste & 724 & 1194 & 1556 & 3999 & 6937 & 15350\\
Sudeste & 991 & 2789 & 5429 & 6922 & 9509 & 29230\\
Nordeste\_Sudeste & 724 & 1403 & 3785 & 5340 & 8306 & 29230\\
\bottomrule
\end{tabular}
\end{table}
...
na Tabela \ref{tab:epas}, juntamente com as medidas dos efeitos de plano
amostral (EPA). Exemplos de utilização da library \texttt{survey} para
obtenção de estimativas apresentadas na \ref{tab:epas} estão na Seção
\ref{epa}. As outras estimativas da Tabela \ref{tab:epas} podem ser
...
Os Capítulos \ref{testqualajust} e \ref{testtab2} tratam da análise de
...
O Capítulo \ref{agregacao-vs-desagregacao} discute algumas formas
alternativas de analisar dados de pesquisas complexas, contrapondo
algumas abordagens distintas à que demos preferência nos capítulos
anteriores, para dar aos leitores condições de apreciar de forma crítica
o material apresentado no restante deste livro. Entre as abordagens
discutidas, há duas principais: a denominada
\texttt{análise\ desagregada}, e a abordagem denominada
\texttt{obtenção\ \ do\ modelo\ amostral} proposta por \citep{PKR}. A
chamada \texttt{análise\ desagregada} incorpora explicitamente na
análise vários aspectos do plano amostral utilizado através do emprego
de modelos hierárquicos \citep{bryk}. Em contraste, a abordagem adotada
nos oito primeiros capítulos é denominada \texttt{análise\ agregada}, e
procura `'eliminar'`da análise efeitos tais como conglomeração induzida
pelo plano amostral, considerando tais efeitos como'`ruídos'`ou fatores
de perturbação que'`atrapalham'' o emprego dos procedimentos clássicos
de estimação, ajuste de modelos e teste de hipóteses.
...
\begin{Highlighting}[]
\KeywordTok{source}\NormalTok{(}\StringTok{"~}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{GitHub}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{adac}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{data}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{ppv1.R"}\NormalTok{)}
\KeywordTok{names}\NormalTok{(ppv1)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# transforma nomes de variáveis para minúsculas}
\KeywordTok{names}\NormalTok{(ppv1)<-}\StringTok{ }\KeywordTok{tolower}\NormalTok{(}\KeywordTok{names}\NormalTok{(ppv1))}
\end{Highlighting}
\end{Shaded}
...
algumas estimativas da Tabela \ref{tab:epas}. Vamos supor que os dados
da pesquisa estão contidos no data frame \texttt{ppv1}, que contém as
...
descritas na \ref{tab:epas}. Os totais das variáveis \texttt{analf1} e
\texttt{analf2} para a região Sudeste fornecem os resultados nas linhas
4) e 5 da Tabela \ref{tab:epas}:
...
O efeito de plano amostral (EPA) de Kish é definido na fórmula 4.1, na
página 54 do livro. Vamos considerar o caso particular em que
...
survey e são distintos daqueles apresentados na Tabela \ref{tab:epas}.
Para obter os valores correspondentes aos da Tabela \ref{tab:epas},
através da library survey, vamos definir as variáveis:
...
library(survey)
source(``\textasciitilde{}\textbackslash{}GitHub\textbackslash{}adac\textbackslash{}data\textbackslash{}ppv1.R'')\#
...
esquemática resumida é apresentada na Tabela \ref{tab:modelclass}.
...
library \texttt{survey} disponível no R, mas o leitor encontrará
funcionalidade semelhante em vários outros pacotes. Nossa escolha se
deveu a dois fatores principais: primeiro ao fato do pacote R ser
aberto, livre e gratuito, dispensando o usuário de custos de
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-17}{}{\label{ex:unnamed-chunk-17}}Efeito
da amostragem estratificada simples com alocação desproporcional
\EndKnitrBlock{example}
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-18}{}{\label{ex:unnamed-chunk-18}}Amostragem
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-19}{}{\label{ex:unnamed-chunk-19}}Matriz
...
\BeginKnitrBlock{remark}
\iffalse {Observação. } \fi É comum aplicar a técnica fazendo o número
de grupos igual ao tamanho da amostra, isto é, tomando \(G=n\) e
...
\protect\hypertarget{def:unnamed-chunk-38}{}{\label{def:unnamed-chunk-38}}O
...
\BeginKnitrBlock{definition}
\protect\hypertarget{def:unnamed-chunk-39}{}{\label{def:unnamed-chunk-39}}O
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-42}{}{\label{ex:unnamed-chunk-42}}População
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-43}{}{\label{ex:unnamed-chunk-43}}\label{exebin}Teste
...
verdadeiros do teste \(T^2\) para o nível nominal de 5\% assumindo
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-49}{}{\label{ex:unnamed-chunk-49}}Uso
...
a \(\mathbf{\theta }\), no sentido definido por \citep{Pfeff}, sobre a
...
\BeginKnitrBlock{definition}
\protect\hypertarget{def:unnamed-chunk-50}{}{\label{def:unnamed-chunk-50}}O
...
{taylor}) e considerando os resultados de \citep{binder83}, podemos
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-51}{}{\label{ex:unnamed-chunk-51}}Análise
...
\(\chi ^{2}\) e sim uma soma ponderada \(\sum_{k}\delta _{k}X_{k}\) de
variáveis \(X_{k}\) IID com distribuição \(\chi ^{2}\left( 1\right)\).
Esse exemplo será rediscutido com mais detalhes na Seção \ref{raoscott}.
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-64}{}{\label{ex:unnamed-chunk-64}}Teste
...
\NormalTok{nestrat <-}\StringTok{ }\KeywordTok{length}\NormalTok{(}\KeywordTok{unique}\NormalTok{(ppv1$ESTRATOF[ppv1$REGIAO==}\DecValTok{2}\NormalTok{]))}
\CommentTok{# número de setores da PPV-sudeste}
\NormalTok{nsetor <-}\StringTok{ }\KeywordTok{length}\NormalTok{(}\KeywordTok{unique}\NormalTok{(ppv1$NSETOR[ppv1$REGIAO==}\DecValTok{2}\NormalTok{]))}
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-85}{}{\label{ex:unnamed-chunk-85}}Correções
...
proporções nas celas da tabela. A Tabela \ref{tab:proplinha} contém, em
cada cela, as estimativas: da proporção na linha, do desvio-padrão da
...
\caption{\label{tab:unnamed-chunk-86}Proporções nas linha, desvios padrões e EPAs de `re` na população}
\centering
\begin{tabular}[t]{lrrr}
\toprule
  & mean & SE & deff\\
...
\caption{\label{tab:unnamed-chunk-87}Proporções nas linha, desvios padrões e EPAs de sx na população}
\centering
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-88}{}{\label{ex:unnamed-chunk-88}}Estimativas
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-97}{}{\label{ex:unnamed-chunk-97}}Plano
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-98}{}{\label{ex:unnamed-chunk-98}}(\citep{bryk},
Cap. 5)
\EndKnitrBlock{example}
