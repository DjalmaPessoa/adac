...
ler os e definir variáveis de interesse por meio de transformação de
...
a library \texttt{survey}, a partir do objeto de \texttt{ppv.se.des} que
incorpora as características do plano amostral utilizado para coletar os
dados. Não é possível estimar diretamente o denominador, pois o plano
amostral AAS (Amostragem Aleatória Simples) não foi adotado na coleta
dos dados. Devemos estimar o denominador a partir de dados obtidos
através do plano amostral VERD, como se eles tivessem sido obtidos
através de AAS.
...
Salva dados para pessoas com 18 ou mais anos de idade
...
O data frame \texttt{pes\_sel} contém as variáveis descritas no
dicionário da PNS e algumas variáveis obtidas por transformação. Os
comandos a seguir ilustram como seriam obtidas as novas variáveis a
partir das variáveis no dicionário da PNS contidas em um data frame
\texttt{x}. Os comandos não precisam ser executados, pois o data frame
lido já contém essas variáveis.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# x data frame só com as variáveis do dicionário}
\CommentTok{# Pessoas que se auto avaliam com saúde boa e muito boa}
\NormalTok{pes_sel <-}\StringTok{ }\KeywordTok{transform}\NormalTok{( x , }\DataTypeTok{saude_b_mb =} \KeywordTok{as.numeric}\NormalTok{( n001 %in%}\StringTok{ }\KeywordTok{c}\NormalTok{( }\StringTok{'1'} \NormalTok{, }\StringTok{'2'} \NormalTok{) ) )}
\CommentTok{# urbano / rural}
\NormalTok{pes_sel <-}\StringTok{ }\KeywordTok{transform}\NormalTok{( x , }\DataTypeTok{situ =} \KeywordTok{factor}\NormalTok{( }\KeywordTok{substr}\NormalTok{( v0024 , }\DecValTok{7} \NormalTok{, }\DecValTok{7} \NormalTok{) , }\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{( }\StringTok{'urbano'} \NormalTok{, }\StringTok{'rural'} \NormalTok{) ) )}

\CommentTok{# sexo}
\NormalTok{pes_sel <-}\StringTok{ }\KeywordTok{transform}\NormalTok{( x , }\DataTypeTok{c006 =} \KeywordTok{factor}\NormalTok{( c006 , }\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{( }\StringTok{'masculino'} \NormalTok{, }\StringTok{'feminino'} \NormalTok{) ) )}

\CommentTok{# uf}
\NormalTok{estado_names <-}\StringTok{ }\KeywordTok{c}\NormalTok{( }\StringTok{"Rondônia"} \NormalTok{, }\StringTok{"Acre"} \NormalTok{, }\StringTok{"Amazonas"} \NormalTok{, }\StringTok{"Roraima"} \NormalTok{, }\StringTok{"Pará"} \NormalTok{, }\StringTok{"Amapá"} \NormalTok{, }\StringTok{"Tocantins"} \NormalTok{, }\StringTok{"Maranhão"} \NormalTok{, }\StringTok{"Piauí"} \NormalTok{, }\StringTok{"Ceará"} \NormalTok{, }\StringTok{"Rio Grande do Norte"} \NormalTok{, }\StringTok{"Paraíba"} \NormalTok{, }\StringTok{"Pernambuco"} \NormalTok{, }\StringTok{"Alagoas"} \NormalTok{, }\StringTok{"Sergipe"} \NormalTok{, }\StringTok{"Bahia"} \NormalTok{, }\StringTok{"Minas Gerais"} \NormalTok{, }\StringTok{"Espírito Santo"} \NormalTok{, }\StringTok{"Rio de Janeiro"} \NormalTok{, }\StringTok{"São Paulo"} \NormalTok{, }\StringTok{"Paraná"} \NormalTok{, }\StringTok{"Santa Catarina"} \NormalTok{, }\StringTok{"Rio Grande do Sul"} \NormalTok{, }\StringTok{"Mato Grosso do Sul"} \NormalTok{, }\StringTok{"Mato Grosso"} \NormalTok{, }\StringTok{"Goiás"} \NormalTok{, }\StringTok{"Distrito Federal"} \NormalTok{)}
\NormalTok{pes_sel <-}\StringTok{ }\KeywordTok{transform}\NormalTok{( x , }\DataTypeTok{uf =} \KeywordTok{factor}\NormalTok{( v0001 , }\DataTypeTok{labels =} \NormalTok{estado_names ) )}

\CommentTok{# região}
\NormalTok{pes_sel <-}\StringTok{ }\KeywordTok{transform}\NormalTok{( x , }\DataTypeTok{region =} \KeywordTok{factor}\NormalTok{( }\KeywordTok{substr}\NormalTok{( v0001 , }\DecValTok{1} \NormalTok{, }\DecValTok{1} \NormalTok{) , }\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{( }\StringTok{"Norte"} \NormalTok{, }\StringTok{"Nordeste"} \NormalTok{, }\StringTok{"Sudeste"} \NormalTok{, }\StringTok{"Sul"} \NormalTok{, }\StringTok{"Centro-Oeste"} \NormalTok{) ) )}

\CommentTok{# recodifica para numérico}
\NormalTok{pes_sel[ , }\KeywordTok{c}\NormalTok{( }\StringTok{'p04101'} \NormalTok{, }\StringTok{'p04102'} \NormalTok{, }\StringTok{'p04301'} \NormalTok{, }\StringTok{'p04302'} \NormalTok{) ] <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{( x[ , }\KeywordTok{c}\NormalTok{( }\StringTok{'p04101'} \NormalTok{, }\StringTok{'p04102'} \NormalTok{, }\StringTok{'p04301'} \NormalTok{, }\StringTok{'p04302'} \NormalTok{) ] , as.numeric )}
\CommentTok{# recodifica deslocamento para trabalho}
\NormalTok{pes_sel <-}
\StringTok{    }\KeywordTok{transform}\NormalTok{(}
        \NormalTok{x ,}
        \DataTypeTok{tempo_desl_trab =} \KeywordTok{ifelse}\NormalTok{( }\KeywordTok{is.na}\NormalTok{( p04101 ) , }\DecValTok{0} \NormalTok{, p04101 *}\StringTok{ }\DecValTok{60} \NormalTok{+}\StringTok{ }\NormalTok{p04102 ) ,}
        \DataTypeTok{tempo_desl_athab =} \KeywordTok{ifelse}\NormalTok{( }\KeywordTok{is.na}\NormalTok{( p04301 ) , }\DecValTok{0} \NormalTok{, p04301 *}\StringTok{ }\DecValTok{60} \NormalTok{+}\StringTok{ }\NormalTok{p04302 ) )}

\NormalTok{pes_sel <-}\StringTok{ }\KeywordTok{transform}\NormalTok{( x , }\DataTypeTok{tempo_desl =} \NormalTok{tempo_desl_trab +}\StringTok{ }\NormalTok{tempo_desl_athab )}

\NormalTok{pes_sel <-}\StringTok{ }\KeywordTok{transform}\NormalTok{( x , }\DataTypeTok{atfi04 =} \KeywordTok{as.numeric}\NormalTok{( tempo_desl >=}\StringTok{ }\DecValTok{30} \NormalTok{) )}


\CommentTok{# idade categórica}
\NormalTok{pes_sel$age_cat <-}\StringTok{ }\KeywordTok{factor}\NormalTok{( }\DecValTok{1} \NormalTok{+}\StringTok{ }\KeywordTok{findInterval}\NormalTok{( }\KeywordTok{as.numeric}\NormalTok{( x$c008 ) , }\KeywordTok{c}\NormalTok{( }\DecValTok{18} \NormalTok{, }\DecValTok{30} \NormalTok{, }\DecValTok{40} \NormalTok{, }\DecValTok{60} \NormalTok{) ) , }\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{( }\StringTok{"0-17"} \NormalTok{, }\StringTok{"18-29"} \NormalTok{, }\StringTok{"30-39"} \NormalTok{, }\StringTok{"40-59"} \NormalTok{, }\StringTok{"60+"} \NormalTok{) )}

\CommentTok{# race}
\NormalTok{pes_sel$raca <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{( pes_sel$c009 )}
\NormalTok{pes_sel[ pes_sel$raca ==}\StringTok{ }\DecValTok{9} \NormalTok{, }\StringTok{'raca'} \NormalTok{] <-}\StringTok{ }\OtherTok{NA}
\NormalTok{pes_sel$raca <-}\StringTok{ }\KeywordTok{factor}\NormalTok{( pes_sel$raca , }\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{( }\StringTok{'Branca'} \NormalTok{, }\StringTok{'Preta'} \NormalTok{, }\StringTok{'Amarela'} \NormalTok{, }\StringTok{'Parda'} \NormalTok{, }\StringTok{'Indígena'} \NormalTok{) )}

\CommentTok{# educação}
\NormalTok{pes_sel$educ <-}\StringTok{ }\KeywordTok{factor}\NormalTok{( }\DecValTok{1} \NormalTok{+}\StringTok{ }\KeywordTok{findInterval}\NormalTok{( }\KeywordTok{as.numeric}\NormalTok{( pes_sel$vdd004 ) , }\KeywordTok{c}\NormalTok{( }\DecValTok{3} \NormalTok{, }\DecValTok{5} \NormalTok{, }\DecValTok{7} \NormalTok{) ) , }\DataTypeTok{labels =} \KeywordTok{c}\NormalTok{( }\StringTok{"SinstFundi"} \NormalTok{, }\StringTok{"FundcMedi"} \NormalTok{, }\StringTok{"MedcSupi"} \NormalTok{, }\StringTok{"Supc"} \NormalTok{) )}

\CommentTok{# número de pessoas no domicílio}
\NormalTok{pes_sel$c001 <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{(pes_sel$c001)}

\CommentTok{# coluna com 1}
\NormalTok{pes_sel$one <-}\StringTok{ }\DecValTok{1}                   
\end{Highlighting}
\end{Shaded}

Criação do objeto de desenho:
...
Totais populacionais usados na pós-estratificação
...
Objeto de desenho pós-estratificado
...
Salva objeto de desenho pós-estratificado

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{saveRDS}\NormalTok{(pes_sel_des_pos,}\StringTok{"C:/adac/PNS/2013 long questionnaire survey design.rds"} \NormalTok{)}
\end{Highlighting}
\end{Shaded}

Vamos agora reproduzir estimativas na Tabela 6.42.1.1 da publicação em
\url{ftp://ftp.ibge.gov.br/PNS/2013/pns2013.pdf}.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class}\NormalTok{(pes_sel$q092)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{table}\NormalTok{(pes_sel$q092)}
\end{Highlighting}
\end{Shaded}
...
se recebe diagnóstico de depressão e zero caso contário:
...
Vamos atualizar o objeto de desenho \texttt{pes\_sel\_des\_pos} para
incluir a variável \texttt{diad\_dep} na componente \texttt{variables}:
...
Um intervalo de confiança aproximado de nível de confiança de 95\% é
dado por
...
O cálculo da estimativa e do intervalo pode ser colocado em uma função
...
Aplicando para o exemplo, obtemos para o país inteiro:
...
\NormalTok{dfsitusex <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{situ.sex =} \KeywordTok{names}\NormalTok{(res)[}\DecValTok{1}\NormalTok{:}\DecValTok{4}\NormalTok{],}\DataTypeTok{prop =} \NormalTok{res[}\DecValTok{1}\NormalTok{:}\DecValTok{4}\NormalTok{], }\DataTypeTok{l.i.c =} \NormalTok{res[}\DecValTok{5}\NormalTok{:}\DecValTok{8}\NormalTok{], }\DataTypeTok{l.s.c=}\NormalTok{res[}\DecValTok{9}\NormalTok{:}\DecValTok{12}\NormalTok{] )}
...
\NormalTok{dfuf <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{uf =} \KeywordTok{names}\NormalTok{(res)[}\DecValTok{1}\NormalTok{:}\DecValTok{27}\NormalTok{], }\DataTypeTok{prop =} \NormalTok{res[}\DecValTok{1}\NormalTok{:}\DecValTok{27}\NormalTok{], }\DataTypeTok{L.I.C. =} \NormalTok{res[}\DecValTok{28}\NormalTok{:}\DecValTok{54}\NormalTok{],}\DataTypeTok{L.S.C =}\NormalTok{res[}\DecValTok{55}\NormalTok{:}\DecValTok{81}\NormalTok{]  )}
...
\NormalTok{dfufsex <-}\StringTok{ }\KeywordTok{data.frame} \NormalTok{(}\DataTypeTok{uf.sex =}\KeywordTok{names}\NormalTok{(res)[}\DecValTok{1}\NormalTok{:}\DecValTok{54}\NormalTok{], }\DataTypeTok{prop =} \NormalTok{res[}\DecValTok{1}\NormalTok{:}\StringTok{ }\DecValTok{54}\NormalTok{], }\DataTypeTok{L.I.C =} \NormalTok{res[}\DecValTok{55}\NormalTok{:}\DecValTok{108}\NormalTok{],}\DataTypeTok{L.S.C=}\NormalTok{res[}\DecValTok{109}\NormalTok{:}\DecValTok{162}\NormalTok{] )}
...
\caption{Representação esquemática da abordagem 1} 
\label{Tab21} 
\begin{tabular}{|c|c|}
\hline\hline
...
\begin{center}
\begin{table}[tbp] \centering
\caption{Representação esquemática da abordagem 2}\bigskip 
\label{Tab22} 
\begin{tabular}{|c|c|}
\hline\hline
...
\begin{center}
\begin{table}[tbp] \centering
\caption{Representação esquemática da abordagem 3}\bigskip 
\label{Tab23} 
\begin{tabular}{|c|c|}
\hline\hline
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-47}{}{\label{ex:unnamed-chunk-47}}Amostragem
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-48}{}{\label{ex:unnamed-chunk-48}}Matriz
...
\iffalse {Observação } \fi A descrição do método \emph{jackknife} aqui
...
\BeginKnitrBlock{remark}
\iffalse {Observação } \fi É comum aplicar a técnica fazendo o número de
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ppv.se<-ppv.se.des.jkn$variables}
\KeywordTok{nrow}\NormalTok{(ppv.se)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{fact.peso.comp<-ppv.se.des.jkn$repweights[[}\DecValTok{1}\NormalTok{]]}
\NormalTok{ind.cong<-ppv.se.des.jkn$repweights[[}\DecValTok{2}\NormalTok{]]}
\NormalTok{mat.fat.pesos<-}\StringTok{ }\NormalTok{fact.peso.comp[ind.cong,]}
\KeywordTok{str}\NormalTok{(mat.fat.pesos)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rep.est.raz<-}\KeywordTok{numeric}\NormalTok{(}\KeywordTok{ncol}\NormalTok{(mat.rep.pesos))}
\NormalTok{for (i in }\DecValTok{1}\NormalTok{:}\KeywordTok{ncol}\NormalTok{(mat.rep.pesos))\{}
\NormalTok{rep.est.raz[i]<-}\KeywordTok{sum}\NormalTok{(mat.rep.pesos[,i]*ppv.se$analf.faixa)/}\KeywordTok{sum}\NormalTok{(mat.rep.pesos[,i]*ppv.se$faixa)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mean.raz<-}\KeywordTok{mean}\NormalTok{( rep.est.raz[ppv.se.des.jkn$rscales>}\DecValTok{0}\NormalTok{])}
\NormalTok{var.jack.raz<-}\StringTok{ }\KeywordTok{sum}\NormalTok{((rep.est.raz-mean.raz)^}\DecValTok{2}\NormalTok{*ppv.se.des.jkn$rscales)*ppv.se.des.jkn$scale}
\KeywordTok{round}\NormalTok{(}\KeywordTok{sqrt}\NormalTok{(var.jack.raz),}\DecValTok{5}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{var.raz.rep<-}\KeywordTok{withReplicates}\NormalTok{(ppv.se.des.jkn, function(w,ppv.se) }\KeywordTok{sum}\NormalTok{(w*ppv.se$analf.faixa)/}\KeywordTok{sum}\NormalTok{(w*ppv.se$faixa))}
\NormalTok{var.raz.rep }
\end{Highlighting}
\end{Shaded}
...
\protect\hypertarget{def:unnamed-chunk-67}{}{\label{def:unnamed-chunk-67}}O
...
O que se observou no Exemplo \ref{ex:epakish} com respeito à dificuldade
de uso dos EPAs de Kish calculados para fins analíticos também se aplica
...
\BeginKnitrBlock{definition}
\protect\hypertarget{def:unnamed-chunk-68}{}{\label{def:unnamed-chunk-68}}O
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:deffuni}{}{\label{ex:deffuni}}Efeitos de plano
...
\begin{center}
\begin{table}[tbp] \centering
\caption{Propriedades dos estimadores de variância do estimador $\bar{y}_{w}$ }
\bigskip \label{tab44} 
\begin{tabular}{|c|l|c|}
\hline\hline
\begin{tabular}{r}
...
\BeginKnitrBlock{remark}
\iffalse {Observação } \fi Neste exemplo não foi feito uso analítico dos
dados e sim descritivo, onde é usual incorporar os pesos no cálculo de
...
\BeginKnitrBlock{remark}
\iffalse {Observação } \fi O exemplo mostra que ignorar a estratificação
ao calcular \(v_{0}\) diminui o \(\mathbf{EPA}\).
\EndKnitrBlock{remark}
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-71}{}{\label{ex:unnamed-chunk-71}}População
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-72}{}{\label{ex:unnamed-chunk-72}}\label{exebin}Teste
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{N<-}\KeywordTok{nrow}\NormalTok{(popul)}
\NormalTok{n1<-}\DecValTok{30}\NormalTok{; n2<-}\DecValTok{30}
\NormalTok{nh=}\KeywordTok{c}\NormalTok{(n1,n2)}
\NormalTok{n<-}\KeywordTok{sum}\NormalTok{(nh)}
\NormalTok{Nh<-}\KeywordTok{table}\NormalTok{(popul$estrat)}
\NormalTok{fh<-nh/Nh ; Wh<-Nh/N ; f<-}\StringTok{ }\NormalTok{n/N}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sal.mat.res<-}\KeywordTok{matrix}\NormalTok{(}\OtherTok{NA}\NormalTok{,}\DecValTok{500}\NormalTok{,}\DecValTok{4}\NormalTok{)}
\NormalTok{rec.mat.res<-}\KeywordTok{matrix}\NormalTok{(}\OtherTok{NA}\NormalTok{,}\DecValTok{500}\NormalTok{,}\DecValTok{4}\NormalTok{)}
\NormalTok{popul$sal <-}\StringTok{ }\NormalTok{popul$sal/}\DecValTok{1000}
\NormalTok{popul$rec <-}\StringTok{ }\NormalTok{popul$rec/}\DecValTok{1000000}
\KeywordTok{library}\NormalTok{(sampling)}
\CommentTok{#Geracão dos dados}
\NormalTok{for(i in }\DecValTok{1}\NormalTok{:}\DecValTok{500}\NormalTok{)\{}
\NormalTok{s<-}\KeywordTok{strata}\NormalTok{(popul, }\StringTok{"estrat"}\NormalTok{,}\KeywordTok{c}\NormalTok{(}\DecValTok{30}\NormalTok{,}\DecValTok{30}\NormalTok{),}\DataTypeTok{method=} \StringTok{"srswor"}\NormalTok{)}
\NormalTok{dados<-}\KeywordTok{getdata}\NormalTok{(popul,s)}
\NormalTok{sal_med_y<-}\KeywordTok{mean}\NormalTok{(dados$sal)}
\NormalTok{rec_med_y<-}\KeywordTok{mean}\NormalTok{(dados$rec)}
\CommentTok{# estimador v0}
\NormalTok{sal_var_aas<-(}\DecValTok{1}\NormalTok{-f)*}\KeywordTok{var}\NormalTok{(dados$sal)/n}
\NormalTok{rec_var_aas<-(}\DecValTok{1}\NormalTok{-f)*}\KeywordTok{var}\NormalTok{(dados$rec)/n}
\CommentTok{# vhat_aes estimador não-viciado}
\NormalTok{desenho<-}\KeywordTok{svydesign}\NormalTok{(~}\DecValTok{1}\NormalTok{,}\DataTypeTok{strata=}\NormalTok{~estrat,}\DataTypeTok{data=}\NormalTok{dados,}\DataTypeTok{fpc=}\NormalTok{~Prob)}
\NormalTok{sal_med_yw<-}\KeywordTok{svymean}\NormalTok{(~sal,desenho)}
\NormalTok{rec_med_yw<-}\KeywordTok{svymean}\NormalTok{(~rec,desenho)}
\NormalTok{sal.mat.res[i,]<-}\KeywordTok{c}\NormalTok{(sal_med_y,}\KeywordTok{coef}\NormalTok{(sal_med_yw),sal_var_aas,}\KeywordTok{SE}\NormalTok{(sal_med_yw)^}\DecValTok{2}\NormalTok{)}
\NormalTok{rec.mat.res[i,]<-}\KeywordTok{c}\NormalTok{(rec_med_y,}\KeywordTok{coef}\NormalTok{(rec_med_yw),rec_var_aas,}\KeywordTok{SE}\NormalTok{(rec_med_yw)^}\DecValTok{2}\NormalTok{)}
\NormalTok{\}}
...
\NormalTok{epa_sal1 <-}\StringTok{ }\NormalTok{sal_res_mean[}\DecValTok{4}\NormalTok{]/sal_res_mean[}\DecValTok{3}\NormalTok{]}
\NormalTok{epa_sal1}
\end{Highlighting}
\end{Shaded}
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-78}{}{\label{ex:unnamed-chunk-78}}Uso
...
\BeginKnitrBlock{definition}
\protect\hypertarget{def:unnamed-chunk-79}{}{\label{def:unnamed-chunk-79}}O
...
pela função \texttt{svyglm}da library survey.
...
\ref{tab63}, obtida pela PROC LOGISTIC do pacote SUDAAN, utiliza a
estatística de Wald baseada no plano amostral com correção. Mais
detalhes são encontrados em Shah et al.(1993).
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{knitr::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{summary}\NormalTok{(inf.logit)$coefficients,}\DataTypeTok{booktabs=}\OtherTok{TRUE}\NormalTok{, }\DataTypeTok{digits=} \KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{3}\NormalTok{,}\DecValTok{2}\NormalTok{,}\DecValTok{3}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-96}{}{\label{ex:unnamed-chunk-96}}Teste
...
\NormalTok{freq <-}\StringTok{ }\KeywordTok{svyby}\NormalTok{(~one, ~idatab, ppv.se.des, unwtd.count)$counts}
...
\NormalTok{tab73 <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{idade=} \NormalTok{idad_tab, }\DataTypeTok{prop_contagem=} \KeywordTok{c}\NormalTok{(}\FloatTok{0.2842}\NormalTok{, }\FloatTok{0.2747}\NormalTok{, }\FloatTok{0.2263}\NormalTok{, }\FloatTok{0.1261}\NormalTok{, }\FloatTok{0.0860}\NormalTok{), frequê}\DataTypeTok{ncia=} \NormalTok{freq, }\DataTypeTok{prop_est_ppv=} \NormalTok{ppv.id$mean,}\DataTypeTok{epa =} \NormalTok{ppv.id$deff) }
\CommentTok{# knitr::kable(tab73, booktabs= TRUE, digits=c(0,4,0,4,4),}
\CommentTok{#   caption = "Vetores de proporções por classes de idade da PPV 96/97 e Contagem}
\CommentTok{# 96 e EPAs calculados para a PPV - Região Sudeste")}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dhat <-}\StringTok{ }\NormalTok{tab73$epa}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{phat <-}\StringTok{ }\NormalTok{tab73$prop_est_ppv}
\NormalTok{deltahatdot <-}\StringTok{ }\KeywordTok{sum}\NormalTok{(phat*(}\DecValTok{1}\NormalTok{-p0)*dhat/(}\DecValTok{4}\NormalTok{*p0))}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{phatv <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(phat[-}\DecValTok{5}\NormalTok{], }\DataTypeTok{ncol=}\DecValTok{1}\NormalTok{)}
\NormalTok{p0v <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(p0[-}\DecValTok{5}\NormalTok{], }\DataTypeTok{ncol=}\DecValTok{1}\NormalTok{)}
\NormalTok{vhat0 <-}\StringTok{ }\NormalTok{vhat[-}\DecValTok{5}\NormalTok{,-}\DecValTok{5}\NormalTok{]}
\NormalTok{x2w <-}\StringTok{  }\KeywordTok{t}\NormalTok{(phatv-p0v)%*%}\StringTok{ }\KeywordTok{solve}\NormalTok{(vhat0)%*%(phatv-p0v)}
\NormalTok{x2w}
\end{Highlighting}
\end{Shaded}
...
\(\alpha =5\%\), diferentemente de todos os outros testes. Os valores
das estatísticas com ajustes de Rao-Scott (com ou sem correção F) são
semelhantes e parecem corrigir exagerademente o \(p\)-valor dos testes.
A estatística de Wald baseada no plano amostral e suas correções F, que
têm valores quase iguais, produzem uma correção menor no \(p\)-valor do
teste. Nesse exemplo, como o número de graus de liberdade (dado pelo
número de unidades primárias na amostra menos o número de
...
\CommentTok{# Estatística de Wald}
\NormalTok{x2_w<-}\KeywordTok{matrix}\NormalTok{((phat-p0)[-}\DecValTok{5}\NormalTok{],}\DataTypeTok{nrow=}\DecValTok{1}\NormalTok{)%*%}\KeywordTok{solve}\NormalTok{(vhat[-}\DecValTok{5}\NormalTok{,-}\DecValTok{5}\NormalTok{])%*%}
\KeywordTok{matrix}\NormalTok{((phat-p0)[-}\DecValTok{5}\NormalTok{],}\DataTypeTok{ncol=}\DecValTok{1}\NormalTok{)}
\NormalTok{x2_w}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{P0<-}\KeywordTok{diag}\NormalTok{(p0)-}\KeywordTok{matrix}\NormalTok{(p0,}\DataTypeTok{ncol=}\DecValTok{1}\NormalTok{)%*%}\KeywordTok{matrix}\NormalTok{(p0,}\DataTypeTok{nrow=}\DecValTok{1}\NormalTok{)}
\NormalTok{x2_p<-n*}\KeywordTok{matrix}\NormalTok{((phat-p0)[-}\DecValTok{5}\NormalTok{],}\DataTypeTok{nrow=}\DecValTok{1}\NormalTok{)%*%}\KeywordTok{solve}\NormalTok{(P0[-}\DecValTok{5}\NormalTok{,-}\DecValTok{5}\NormalTok{])%*%}
\KeywordTok{matrix}\NormalTok{((phat-p0)[-}\DecValTok{5}\NormalTok{],}\DataTypeTok{ncol=}\DecValTok{1}\NormalTok{)}
\NormalTok{x2_p}
\end{Highlighting}
\end{Shaded}
...
Exemplo \ref{exe:Analisec} do Capítulo \ref{ajmodpar}. Se calcularmos as
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-117}{}{\label{ex:unnamed-chunk-117}}Correções
de EPA médio das estatísticas \(X_{P}^{2}\left(I\right)\) e
\(X_{P}^{2}\left(H\right)\).
\EndKnitrBlock{example}

Considerando os dados do Exemplo \ref{ex:pnad90} do Capítulo
\ref{modreg}, vamos testar a hipótese de independência entre as
variáveis Sexo (sx) e Rendimento médio mensal (re). Vamos fazer também
um teste de homogeneidade, para comparar as distribuições de renda para
os dois sexos.
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-120}{}{\label{ex:unnamed-chunk-120}}Estimativas
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-129}{}{\label{ex:unnamed-chunk-129}}Plano
...
análise são apresentados na Tabela \ref{tab91}.
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-130}{}{\label{ex:unnamed-chunk-130}}(\citep{bryk},
Cap. 5)
\EndKnitrBlock{example}
