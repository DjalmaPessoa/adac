...
\BeginKnitrBlock{example}<div class="example"><span class="example" id="exm:exe12"><strong>(\#exm:exe12) </strong></span>Utilização da library survey do R para estimar alguns totais e razões na Tabela \@ref(tab:epas)</div>\EndKnitrBlock{example}
...
As variáveis que definem estratos, conglomerados e pesos na `PPV` são respectivamente, `estratof`, `nsetor` e `pesof`. O objeto de desenho amostral, `ppv.plan` incorpora as informações do plano amostral adotado na `PPV`.
...
ppv.plan<-svydesign(ids = ~nsetor, strata = ~estratof,
data = ppv1.dat, nest = TRUE, weights = ~pesof)
```
...
```r
ppv.se.plan <- subset(ppv.plan, regiao == 2)
```
...
```r
svytotal(~analf1, ppv.se.plan, deff = TRUE)
```
...
```r
svytotal(~analf2, ppv.se.plan, deff = TRUE)
```
...
```r
svyratio(~analf1, ~faixa1, ppv.se.plan)
```
...
```r
svyratio(~analf2, ~faixa2, ppv.se.plan)
```
...
```r
svyby(~analf1, ~regiao, ppv.plan, svytotal, deff = TRUE)
```
...
```r
ppv.plan <- ppv.dat %>% as_survey_design (ids = nsetor, strata = estratof,
...
e salvá-los em um data frame `pnadc032016.dat`.


```r
lodown( "pnadc" , subset( pnadc_cat , year == 2016 & quarter == '03' ) )
pnadc032016.dat <- readRDS( paste0( tempdir() , "/pnadc 2016 03.rds" ) )
```
...
```r
saveRDS(pnadc032016.dat, file="C:/adac/pnadc/pnadc 2016 03.rds")
```


Partindo do arquivo `pnadc 2016 03.rds`, podemos recuperar o data frame `pnadc032016.dat`: 

 

```r
pnadc032016.dat <- readRDS("C:/adac/pnadc/pnadc 2016 03.rds")
```
...
```r
pnadc032016.plan <- svydesign(ids =~upa, strata=~estrato, 
  weights=~v1027, data = pnadc032016.dat, nest=TRUE)
```
...
```r
df_pos <-data.frame(posest=unique(pnadc032016.dat$posest), 
  Freq=unique(pnadc032016.dat$v1029))
```
-   Pós-estratifica objeto de desenho inicial:


```r
pnadc032016.calib <-postStratify(pnadc032016.plan, ~posest, df_pos)
```
...
taxa_des <- svyratio(~ vd4002=="2" ,
  ~ vd4001 == "1" , pnadc032016.calib , na.rm = TRUE)
# organiza saída
result <- data.frame(
  100*coef(taxa_des),
  100*SE(taxa_des), 
...
Inicialmente, vamos estimar características de pessoas com 18 anos ou mais mais que responderam o questionário longo e salvar os microdados dessa amostra no data frame `pes_sel.dat`.


```r
pes_sel.dat <- readRDS("C:/adac/PNS/2013 long questionnaire survey.rds")
dim(pes_sel.dat)
```
...
O data frame `pes_sel.dat` contém as variáveis descritas no dicionário da `PNS` e algumas variáveis obtidas derivadas. 
...
Os pesos do objeto de desenho `pes_sel.plan` devem ser modificados de modo que as estimativas dos totais populacionais dos pós-estratos fixados coincidam com os  totais populacionais dos pós-estratos conhecidos a partir do Censo Demográfico. O data frame `post_pop` contém na primeira coluna a identidade dos pós-estratos e na segunda seus totais populacionais.


```r
post_pop <- unique( pes_sel.dat[ c( 'v00293.y' , 'v00292.y' ) ] )
names( post_pop ) <- c( "v00293.y" , "Freq" )
```

Utilizando a função `postStratify()` da library `survey` [@R-survey] incorpora-se no objeto de desenho `pes_sel.plan` as informações contidas no data frame `post_pop`.



```r
pes_sel.calib <- postStratify( pes_sel.plan , ~v00293.y , post_pop )
```
...
```r
diag_dep <- as.numeric (pes_sel.dat$q092=="1")
```

Como mencionado, o objeto de desenho pós-estratificado `pes_sel.calib` pode ser lido
do arquivo `2013 all questionnaire survey design.rds`. Vamos atualizar o objeto de desenho `pes_sel.calib` para incluir a variável `diad_dep` na componente `variables`:


```r
pes_sel.calib <- update (pes_sel.calib, diag_dep = diag_dep )
```
...
```r
diagdepbr <- svymean(~diag_dep, pes_sel.calib)
...
\caption{(\#tab:unnamed-chunk-41)Proporção de diagnóstico de depressão por sexo}
\centering
...
\caption{(\#tab:unnamed-chunk-42)Proporção de diagnóstico de depressão por situação}
\centering
\begin{tabular}[t]{lrrr}
\toprule
...
\caption{(\#tab:unnamed-chunk-44)Proporção de diagnóstico de depressão por uf}
\centering
\begin{tabular}[t]{lrrr}
\toprule
uf & prop & l.i.c. & l.s.c\\
...
```r
pes_all.calib <- readRDS("C:/adac/PNS/2013 all questionnaire survey design.rds")
```
...
\caption{(\#tab:unnamed-chunk-50)Proporção de pessoas com seguro de saúde por nível de instrução}
\centering
\begin{tabular}[t]{lrrr}
\toprule
  & Prop & l.i.c. & l.s.c.\\
...
![](01-Cap1_files/figure-latex/unnamed-chunk-51-1.pdf)<!-- --> 
...
Consideremos o plano amostral `ppv.se.plan` anteriormente definido. 
Vamos usar a função `Vwaas` para obter uma estimativa da variância do total estimado da variável `analf1`. Todos os elementos os elementos necessários estão contidos no objeto `ppv.se.plan`:



```r
VAAS1<- Vwaas(ppv.se.plan$variables[,"analf1"],weights(ppv.se.plan))
VAAS2<- Vwaas(ppv.se.plan$variables[,"analf2"],weights(ppv.se.plan))
```
...
```r
attr(svytotal(~analf1, ppv.se.plan),"var")/VAAS1
```
...
```r
attr(svytotal(~analf2, ppv.se.plan),"var")/VAAS2
```
...
```r
analf1.se<-with(ppv1.dat,((v04a01==2|v04a02==2) & (v02a08>=7&v02a08<=14))&(regiao==2))
analf2.se<-with(ppv1.dat,((v04a01==2|v04a02==2) & (v02a08>14))&(regiao==2))
ppv.plan <- update (ppv.plan,analf1.se=analf1.se,analf2.se=analf2.se  )
svytotal(analf1.se,ppv.plan,deff=T)
```
...
```r
svytotal(analf2.se,ppv.plan,deff=T)
```
...
```r
svytotal(~I(ifelse(regiao==2,analf1,0)),ppv.plan,deff=T)
```
...
```r
svytotal(~I(ifelse(regiao==2,analf2,0)),ppv.plan,deff=T)
```
...
são iguais quando usamos: a variável  `analf1.se` com o objeto de desenho `ppv.plan` 
ou a variável `analf1` com o objeto `ppv.se.plan`. Porém na estimativa do denominador do EPA, obtida a partir da função `Vwaas`, 
