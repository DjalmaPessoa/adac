...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ppv.des <-}\StringTok{ }\NormalTok{ppv }\OperatorTok{%>%}\StringTok{ }\KeywordTok{as_survey_design}\NormalTok{ (}\DataTypeTok{ids =}\NormalTok{ nsetor, }\DataTypeTok{strata =}\NormalTok{ estratof,}
...
\includegraphics{01-Cap1_files/figure-latex/unnamed-chunk-56-1.pdf}
...
\emph{alocação igual}, pois o tamanho total da amostra é repartido em
...
\endhead
x & 0 & 1 & Total\tabularnewline
0 & \(\eta_{00}\)/\(\eta_{0+}\) & \(\eta_{01}\)/\(\eta_{0+}\) &
1\tabularnewline
1 & \(\eta_{10}\)/\(\eta_{1+}\) & \(\eta_{11}\)/\(\eta_{1+}\) &
1\tabularnewline
\bottomrule
\end{longtable}
...
Média\_de\_ybar\_sobre\_ 500\_ amostras & 160.72319 & 4.289349\\
Média\_de\_ybarw\_sobre\_500\_amostras & 76.67527 & 2.121696\\
\bottomrule
\end{tabular}
...
Média\_de\_ Vhat\_AES(yvarw) & 232.7219 & 0.4382666\\
Valor\_esperado\_de\_v0\_usando\_população & NA & NA\\
Média\_de\_v0\_usando\_500\_amostras & 1643.3960 & 1.3296574\\
\bottomrule
\end{tabular}
