...
Os pesos do objeto de desenho \texttt{pes\_sel\_plan} devem ser
...
\texttt{pes\_sel\_plan} as informações contidas no data frame
\texttt{post\_pop}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pes_sel_calib <-}\StringTok{ }\KeywordTok{postStratify}\NormalTok{( pes_sel_plan , }\OperatorTok{~}\NormalTok{v00293.y , post_pop )}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{saveRDS}\NormalTok{(pes_sel_calib, }\DataTypeTok{file =} \StringTok{"C:/adac/PNS/pns_des_sel.rds"}\NormalTok{ )}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pes_sel_calib <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{(}\StringTok{"C:/adac/PNS/pns_des_sel.rds"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\texttt{pes\_sel\_calib} já se encontra disponível no arquivo 1
...
\texttt{pes\_sel\_calib} pode ser lido do arquivo
\texttt{2013\ all\ questionnaire\ survey\ design.rds}. Vamos atualizar o
objeto de desenho \texttt{pes\_sel\_calib} para incluir a variável
\texttt{diad\_dep} na componente \texttt{variables}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pes_sel_calib <-}\StringTok{ }\KeywordTok{update}\NormalTok{ (pes_sel_calib, }\DataTypeTok{diag_dep =}\NormalTok{ diag_dep )}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{diagdepbr <-}\StringTok{ }\KeywordTok{svymean}\NormalTok{(}\OperatorTok{~}\NormalTok{diag_dep, pes_sel_calib)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pes_all_calib <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{(}\StringTok{"C:/adac/PNS/2013 all questionnaire survey design.rds"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Média\_de\_ybar\_sobre\_ 500\_ amostras & 162.33292 & 4.228982\\
Média\_de\_ybarw\_sobre\_500\_amostras & 77.42824 & 2.109680\\
\bottomrule
\end{tabular}
...
Média\_de\_ Vhat\_AES(yvarw) & 242.8132 & 0.4614696\\
Valor\_esperado\_de\_v0\_usando\_população & NA & NA\\
Média\_de\_v0\_usando\_500\_amostras & 1716.4542 & 1.2691237\\
\bottomrule
\end{tabular}
