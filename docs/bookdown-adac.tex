...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ppv.des <-}\StringTok{ }\NormalTok{ppv1 }\OperatorTok{%>%}\StringTok{ }\KeywordTok{as_survey_design}\NormalTok{ (}\DataTypeTok{ids =}\NormalTok{ nsetor, }\DataTypeTok{strata =}\NormalTok{ estratof,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ppv.des <-}\StringTok{ }\NormalTok{ppv.des }\OperatorTok{%>%}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{res <-}\StringTok{ }\NormalTok{ppv.des }\OperatorTok{%>%}\StringTok{  }
...
\NormalTok{)}
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{as_tibble}\NormalTok{(res), }\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{format =} \StringTok{"latex"}\NormalTok{, }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{VAAS1<-}\StringTok{ }\KeywordTok{Vwaas}\NormalTok{(ppv.se.des}\OperatorTok{$}\NormalTok{variables[,}\StringTok{"analf1"}\NormalTok{],}\KeywordTok{weights}\NormalTok{(ppv.se.des))}
\NormalTok{VAAS2<-}\StringTok{ }\KeywordTok{Vwaas}\NormalTok{(ppv.se.des}\OperatorTok{$}\NormalTok{variables[,}\StringTok{"analf2"}\NormalTok{],}\KeywordTok{weights}\NormalTok{(ppv.se.des))}
\end{Highlighting}
\end{Shaded}
...
}Utilização da library survey do R para estimar taxa de desocupação para
um trimestre na PNADC
\EndKnitrBlock{example}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{df_pos <-}\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{posest=}\KeywordTok{unique}\NormalTok{(x}\OperatorTok{$}\NormalTok{posest), }
  \DataTypeTok{Freq=}\KeywordTok{unique}\NormalTok{(x}\OperatorTok{$}\NormalTok{v1029))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{diag_dep <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{ (pes_sel}\OperatorTok{$}\NormalTok{q092}\OperatorTok{==}\StringTok{"1"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{diagdepbr <-}\StringTok{ }\KeywordTok{svymean}\NormalTok{(}\OperatorTok{~}\NormalTok{diag_dep, pes_sel_des_pos)}
...
\includegraphics{01-Cap1_files/figure-latex/unnamed-chunk-60-1.pdf}
...
amostral pode viciar a inferência. Veja o Exemplo \ref{ex:nonigno}
adiante.
...
}População conglomerada com conglomerados de tamanho 2 \citep{SHS89},
p.~25
\EndKnitrBlock{example}
...
\BeginKnitrBlock{example}
\protect\hypertarget{exm:unnamed-chunk-1}{}{\label{exm:unnamed-chunk-1} }Uso
...
s}\sum\limits_{j\in s}\frac{1}{\pi _{ij}}\frac{\pi _{ij}-\pi _{i}\pi _{j}}{\pi _{i}\pi _{j}}
\left[ \mathbf{u}_{i}\left( \mathbf{\hat{\theta}}_{\pi }\right) \right]
\left[ \mathbf{u}_{j}\left( \mathbf{\hat{\theta}}_{\pi }\right) \right]^{^{\prime }}  
\label{eq:modpar14}
\end{equation}
...
\BeginKnitrBlock{example}
\protect\hypertarget{exm:pnad}{}{\label{exm:pnad} }Análise do perfil
...
\BeginKnitrBlock{example}
\protect\hypertarget{exm:unnamed-chunk-1}{}{\label{exm:unnamed-chunk-1}
}Correções de EPA médio das estatísticas \(X_{P}^{2}\left(I\right)\) e
\(X_{P}^{2}\left(H\right)\).
\EndKnitrBlock{example}
...
\BeginKnitrBlock{example}
\protect\hypertarget{exm:unnamed-chunk-1}{}{\label{exm:unnamed-chunk-1}
}Plano amostral de pesquisa educacional (\citep{lethonen}, p.~297).
