...
\NormalTok{ppv1_dat <-}\StringTok{ }\KeywordTok{transform}\NormalTok{(ppv_dat, }
...
\texttt{pesof}. O objeto de desenho amostral, \texttt{ppv\_plan}
incorpora as informações do plano amostral adotado na \texttt{PPV}.
...
\DataTypeTok{data =}\NormalTok{ ppv1_dat, }\DataTypeTok{nest =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{weights =} \OperatorTok{~}\NormalTok{pesof)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ppv_plan <-}\StringTok{ }\NormalTok{ppv_dat }\OperatorTok{%>%}\StringTok{ }\KeywordTok{as_survey_design}\NormalTok{ (}\DataTypeTok{ids =}\NormalTok{ nsetor, }\DataTypeTok{strata =}\NormalTok{ estratof,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{res <-}\StringTok{ }\NormalTok{ppv_plan }\OperatorTok{%>%}\StringTok{  }
\KeywordTok{group_by}\NormalTok{(regiao) }\OperatorTok{%>%}\StringTok{ }
...
\NormalTok{)}
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{as.data.frame}\NormalTok{(res), }\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{digits =} \DecValTok{3}\NormalTok{, }
...
microdados e salvá-los em um data frame \texttt{pnadc032016\_dat}.
...
\NormalTok{pnadc032016_dat <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{( }\KeywordTok{paste0}\NormalTok{( }\KeywordTok{tempdir}\NormalTok{() , }\StringTok{"/pnadc201603.rds"}\NormalTok{ ) )}
\end{Highlighting}
\end{Shaded}

vamos salvar o data frame \texttt{pnadc032016\_dat} para uso posterior,
:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{saveRDS}\NormalTok{(pnadc032016_dat, }\DataTypeTok{file=}\StringTok{"C:/adac/pnadc/pnadc201603.rds"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Partindo do arquivo \texttt{pnadc201603.rds}, podemos recuperar o data
frame \texttt{pnadc032016\_dat}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pnadc032016_dat <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{(}\StringTok{"C:/adac/pnadc/pnadc201603.rds"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pnadc032016_plan <-}\StringTok{ }\KeywordTok{svydesign}\NormalTok{(}\DataTypeTok{ids =}\OperatorTok{~}\NormalTok{upa, }\DataTypeTok{strata=}\OperatorTok{~}\NormalTok{estrato, }
  \DataTypeTok{weights=}\OperatorTok{~}\NormalTok{v1027, }\DataTypeTok{data =}\NormalTok{ pnadc032016_dat, }\DataTypeTok{nest=}\OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{df_pos <-}\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{posest=}\KeywordTok{unique}\NormalTok{(pnadc032016_dat}\OperatorTok{$}\NormalTok{posest), }
  \DataTypeTok{Freq=}\KeywordTok{unique}\NormalTok{(pnadc032016_dat}\OperatorTok{$}\NormalTok{v1029))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pnadc032016_calib <-}\KeywordTok{postStratify}\NormalTok{(pnadc032016_plan, }\OperatorTok{~}\NormalTok{posest, df_pos)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{taxa_des <-}\StringTok{ }\KeywordTok{svyratio}\NormalTok{(}\OperatorTok{~}\StringTok{ }\NormalTok{vd4002}\OperatorTok{==}\StringTok{"2"}\NormalTok{ ,}
  \OperatorTok{~}\StringTok{ }\NormalTok{vd4001 }\OperatorTok{==}\StringTok{ "1"}\NormalTok{ , pnadc032016_calib , }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{)}
...
dessa amostra no data frame \texttt{pns\_dat}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pns_dat <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{(}\StringTok{"C:/adac/PNS/2013 long questionnaire survey.rds"}\NormalTok{)}
\KeywordTok{dim}\NormalTok{(pns_dat)}
\end{Highlighting}
\end{Shaded}
...
\BeginKnitrBlock{remark}
\iffalse{} {Observação. } \fi{}No data frame pns\_dat o nome das
variáveis, obtidos por \texttt{names(pns)}, estão em minúsculas. No
dicionário da PNS os códigos correspondentes estão em maiúsculas.
\EndKnitrBlock{remark} O data frame \texttt{pns\_dat} contém as
...
\StringTok{    }\KeywordTok{svydesign}\NormalTok{(}
        ...
        \DataTypeTok{data =}\NormalTok{ pns_dat ,}
...
Os pesos do objeto de desenho \texttt{pns\_plan} devem ser modificados
de modo que as estimativas dos totais populacionais dos pós-estratos
fixados coincidam com os totais populacionais dos pós-estratos
conhecidos a partir do Censo Demográfico. O data frame
\texttt{post\_pop} contém na primeira coluna a identidade dos
pós-estratos e na segunda seus totais populacionais.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{post_pop <-}\StringTok{ }\KeywordTok{unique}\NormalTok{( pns_dat[ }\KeywordTok{c}\NormalTok{( }\StringTok{'v00293.y'}\NormalTok{ , }\StringTok{'v00292.y'}\NormalTok{ ) ] )}
...
\citep{R-survey} incorpora-se no objeto de desenho \texttt{pns\_plan} as
informações contidas no data frame \texttt{post\_pop}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pns_calib <-}\StringTok{ }\KeywordTok{postStratify}\NormalTok{( pns_plan , }\OperatorTok{~}\NormalTok{v00293.y , post_pop )}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{saveRDS}\NormalTok{(pns_calib, }\DataTypeTok{file =} \StringTok{"C:/adac/PNS/pns_calib.rds"}\NormalTok{ )}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pns_calib <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{(}\StringTok{"C:/adac/PNS/pns_calib.rds"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\texttt{pns\_calib} já se encontra disponível no arquivo 1
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{diag_dep <-}\StringTok{ }\KeywordTok{as.numeric}\NormalTok{ (pns_dat}\OperatorTok{$}\NormalTok{q092}\OperatorTok{==}\StringTok{"1"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

Como mencionado, o objeto de desenho pós-estratificado
\texttt{pns\_calib} pode ser lido do arquivo
\texttt{2013\ all\ questionnaire\ survey\ design.rds}. Vamos atualizar o
objeto de desenho \texttt{pns\_calib} para incluir a variável
\texttt{diad\_dep} na componente \texttt{variables}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pns_calib <-}\StringTok{ }\KeywordTok{update}\NormalTok{ (pns_calib, }\DataTypeTok{diag_dep =}\NormalTok{ diag_dep )}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{diagdepbr <-}\StringTok{ }\KeywordTok{svymean}\NormalTok{(}\OperatorTok{~}\NormalTok{diag_dep, pns_calib)}
\CommentTok{# estimativa e erro padrão, ambos em % }
\KeywordTok{round}\NormalTok{(}\DecValTok{100} \OperatorTok{*}\StringTok{ }\KeywordTok{coef}\NormalTok{(diagdepbr),}\DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{round}\NormalTok{(}\DecValTok{100} \OperatorTok{*}\StringTok{ }\KeywordTok{SE}\NormalTok{(diagdepbr)  ,}\DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{tab:unnamed-chunk-41}Proporção de diagnóstico de depressão por sexo}
\centering
...
\caption{\label{tab:unnamed-chunk-42}Proporção de diagnóstico de depressão por situação}
\centering
\begin{tabular}[t]{lrrr}
\toprule
...
\caption{\label{tab:unnamed-chunk-44}Proporção de diagnóstico de depressão por uf}
\centering
\begin{tabular}[t]{lrrr}
\toprule
uf & prop & l.i.c. & l.s.c\\
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pns_all_calib <-}\StringTok{ }\KeywordTok{readRDS}\NormalTok{(}\StringTok{"C:/adac/PNS/2013 all questionnaire survey design.rds"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\DataTypeTok{design =}\NormalTok{ pns_all_des_pos , }\DataTypeTok{vartype =} \StringTok{"ci"}\NormalTok{ ,  }\DataTypeTok{level =} \FloatTok{0.95}\NormalTok{ ,}
...
\caption{\label{tab:unnamed-chunk-50}Proporção de pessoas com seguro de saúde por nível de instrução}
\centering
\begin{tabular}[t]{lrrr}
\toprule
  & Prop & l.i.c. & l.s.c.\\
...
\NormalTok{    byeduc,}
    \KeywordTok{aes}\NormalTok{( }\DataTypeTok{x =} \KeywordTok{c}\NormalTok{(}\StringTok{"SinstFundi"}\NormalTok{, }\StringTok{"FundcMedi"}\NormalTok{, }\StringTok{"MedcSupi"}\NormalTok{, }\StringTok{"Supc"}\NormalTok{) , }\DataTypeTok{y =} \DecValTok{100}\OperatorTok{*}\NormalTok{Prop )}
\NormalTok{    ) }\OperatorTok{+}
\StringTok{    }\KeywordTok{geom_bar}\NormalTok{( }\DataTypeTok{stat =} \StringTok{"identity"}\NormalTok{ ) }\OperatorTok{+}
\StringTok{    }\KeywordTok{geom_errorbar}\NormalTok{( }\KeywordTok{aes}\NormalTok{( }\DataTypeTok{ymin =} \DecValTok{100}\OperatorTok{*}\NormalTok{l.i.c. , }\DataTypeTok{ymax =} \DecValTok{100}\OperatorTok{*}\NormalTok{l.s.c. ))  }\OperatorTok{+}
\StringTok{  }\KeywordTok{ylim}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{,}\DecValTok{100}\NormalTok{))}\OperatorTok{+}
\StringTok{    }\KeywordTok{xlab}\NormalTok{( }\StringTok{"nível de instrução"}\NormalTok{ ) }\OperatorTok{+}
\StringTok{    }\KeywordTok{ylab}\NormalTok{( }\StringTok{"% com seguro de saúde" )+}
\StringTok{  ggtitle("}\NormalTok{% de pessoas com plano de saúde, com indicação do intervalo de}
\NormalTok{  confiança de }\DecValTok{95}\NormalTok{%, segundo grupos de nível de instrução }\OperatorTok{-}\StringTok{ }\NormalTok{Brasil }\OperatorTok{-}\StringTok{ }\DecValTok{2013}\StringTok{")}
\end{Highlighting}
\end{Shaded}

\includegraphics{01-Cap1_files/figure-latex/unnamed-chunk-51-1.pdf}
...
\texttt{ppv\_se\_plan} que incorpora as características do plano
amostral utilizado para coletar os dados. Não é possível estimar
diretamente o denominador, pois o plano amostral AAS (Amostragem
Aleatória Simples) não foi adotado na coleta dos dados. Devemos estimar
o denominador a partir de dados obtidos através do plano amostral VERD,
como se eles tivessem sido obtidos através de AAS.
...
\texttt{ppv\_se\_plan} anteriormente definido. Vamos usar a função
...
estão contidos no objeto \texttt{ppv\_se\_plan}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{VAAS1<-}\StringTok{ }\KeywordTok{Vwaas}\NormalTok{(ppv_se_plan}\OperatorTok{$}\NormalTok{variables[,}\StringTok{"analf1"}\NormalTok{],}\KeywordTok{weights}\NormalTok{(ppv_se_plan))}
\NormalTok{VAAS2<-}\StringTok{ }\KeywordTok{Vwaas}\NormalTok{(ppv_se_plan}\OperatorTok{$}\NormalTok{variables[,}\StringTok{"analf2"}\NormalTok{],}\KeywordTok{weights}\NormalTok{(ppv_se_plan))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{attr}\NormalTok{(}\KeywordTok{svytotal}\NormalTok{(}\OperatorTok{~}\NormalTok{analf1, ppv_se_plan),}\StringTok{"var"}\NormalTok{)}\OperatorTok{/}\NormalTok{VAAS1}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{attr}\NormalTok{(}\KeywordTok{svytotal}\NormalTok{(}\OperatorTok{~}\NormalTok{analf2, ppv_se_plan),}\StringTok{"var"}\NormalTok{)}\OperatorTok{/}\NormalTok{VAAS2}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{analf1.se<-}\KeywordTok{with}\NormalTok{(ppv1_dat,((v04a01}\OperatorTok{==}\DecValTok{2}\OperatorTok{|}\NormalTok{v04a02}\OperatorTok{==}\DecValTok{2}\NormalTok{) }\OperatorTok{&}\StringTok{ }\NormalTok{(v02a08}\OperatorTok{>=}\DecValTok{7}\OperatorTok{&}\NormalTok{v02a08}\OperatorTok{<=}\DecValTok{14}\NormalTok{))}\OperatorTok{&}\NormalTok{(regiao}\OperatorTok{==}\DecValTok{2}\NormalTok{))}
\NormalTok{analf2.se<-}\KeywordTok{with}\NormalTok{(ppv1_dat,((v04a01}\OperatorTok{==}\DecValTok{2}\OperatorTok{|}\NormalTok{v04a02}\OperatorTok{==}\DecValTok{2}\NormalTok{) }\OperatorTok{&}\StringTok{ }\NormalTok{(v02a08}\OperatorTok{>}\DecValTok{14}\NormalTok{))}\OperatorTok{&}\NormalTok{(regiao}\OperatorTok{==}\DecValTok{2}\NormalTok{))}
\NormalTok{ppv_plan <-}\StringTok{ }\KeywordTok{update}\NormalTok{ (ppv_plan,}\DataTypeTok{analf1.se=}\NormalTok{analf1.se,}\DataTypeTok{analf2.se=}\NormalTok{analf2.se  )}
\KeywordTok{svytotal}\NormalTok{(analf1.se,ppv_plan,}\DataTypeTok{deff=}\NormalTok{T)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{svytotal}\NormalTok{(analf2.se,ppv_plan,}\DataTypeTok{deff=}\NormalTok{T)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{svytotal}\NormalTok{(}\OperatorTok{~}\KeywordTok{I}\NormalTok{(}\KeywordTok{ifelse}\NormalTok{(regiao}\OperatorTok{==}\DecValTok{2}\NormalTok{,analf1,}\DecValTok{0}\NormalTok{)),ppv_plan,}\DataTypeTok{deff=}\NormalTok{T)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{svytotal}\NormalTok{(}\OperatorTok{~}\KeywordTok{I}\NormalTok{(}\KeywordTok{ifelse}\NormalTok{(regiao}\OperatorTok{==}\DecValTok{2}\NormalTok{,analf2,}\DecValTok{0}\NormalTok{)),ppv_plan,}\DataTypeTok{deff=}\NormalTok{T)}
\end{Highlighting}
\end{Shaded}
...
\texttt{analf1.se} com o objeto de desenho \texttt{ppv\_plan} ou a
variável \texttt{analf1} com o objeto \texttt{ppv\_se\_plan}. Porém na
estimativa do denominador do EPA, obtida a partir da função
\texttt{Vwaas}, obtemos resultados diferentes quando usamos
\texttt{analf1.se} ou \texttt{analf1},\\
com os pesos correspondentes. No segundo caso, a soma dos pesos não
estima \(N\). Deve-se ter o cuidado, quando estimamos em um domínio, de
trabalhar com pesos cuja soma seja um estimador do tamanho da população.
...
\begin{eqnarray*}
\mathbf{EPA}\left(\overline{SAL}_{w},v_{0}\left(\overline{SAL}_{w}\right)\right) &=&\frac{231.84}{1636.1}=0.142 \\
...
\right) &=&\frac{244.18}{1613.3}=0.151\mbox{ e } \\
\mathbf{EPA}\left( \overline{REC}_{w},v_{0}\left( \overline{REC}_{w}\right)
...
Utilizando o R, obtemos a seguir alguns resultados descritos no Exemplo
\ref{exm:deffuni}.
...
\KeywordTok{names}\NormalTok{(popul_dat)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## [1] "id"     "po"     "sal"    "rec"    "estrat"
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{N<-}\KeywordTok{nrow}\NormalTok{(popul_dat)}
...
\NormalTok{Nh<-}\KeywordTok{table}\NormalTok{(popul_dat}\OperatorTok{$}\NormalTok{estrat)}
\NormalTok{fh<-nh}\OperatorTok{/}\NormalTok{Nh ; Wh<-Nh}\OperatorTok{/}\NormalTok{N ; f<-}\StringTok{ }\NormalTok{n}\OperatorTok{/}\NormalTok{N}
\end{Highlighting}
\end{Shaded}

Na simulação, usamos a library \texttt{sampling} \citep{R-sampling} para
gerar amostras estratificadas de tamanho 30, com estratos definidos na
Tabela \ref{tab:estempr}, para reproduzir os elementos das Tabelas
\ref{tab:proestmed} e \ref{tab:varest}.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# muda escala de salários e receitas}
...
\NormalTok{popul_dat}\OperatorTok{$}\NormalTok{rec <-}\StringTok{ }\NormalTok{popul_dat}\OperatorTok{$}\NormalTok{rec}\OperatorTok{/}\DecValTok{1000000}
...
\NormalTok{  s<-}\KeywordTok{strata}\NormalTok{(popul_dat, }\StringTok{"estrat"}\NormalTok{,}\KeywordTok{c}\NormalTok{(}\DecValTok{30}\NormalTok{,}\DecValTok{30}\NormalTok{),}\DataTypeTok{method=} \StringTok{"srswor"}\NormalTok{)}
\NormalTok{dados<-}\KeywordTok{getdata}\NormalTok{(popul_dat,s)}
\CommentTok{# média amostral de salário e de receita}
\NormalTok{est_aas <-}\StringTok{ }\NormalTok{est_aas}\OperatorTok{+}\KeywordTok{c}\NormalTok{(}\KeywordTok{mean}\NormalTok{(dados}\OperatorTok{$}\NormalTok{sal), }\KeywordTok{mean}\NormalTok{(dados}\OperatorTok{$}\NormalTok{rec))}
\CommentTok{# estimador v0}
\NormalTok{cov_mat_aas_est <-}\StringTok{ }\NormalTok{cov_mat_aas_est }\OperatorTok{+}\StringTok{ }\NormalTok{(}\DecValTok{1}\OperatorTok{-}\NormalTok{f)}\OperatorTok{*}\KeywordTok{cov}\NormalTok{(}\KeywordTok{cbind}\NormalTok{(dados}\OperatorTok{$}\NormalTok{sal, dados}\OperatorTok{$}\NormalTok{rec))}\OperatorTok{/}\NormalTok{n}

\CommentTok{# vhat_aes estimador não-viciado}
\NormalTok{popul_plan<-}\KeywordTok{svydesign}\NormalTok{(}\OperatorTok{~}\DecValTok{1}\NormalTok{,}\DataTypeTok{strata=}\OperatorTok{~}\NormalTok{estrat,}\DataTypeTok{data=}\NormalTok{dados,}\DataTypeTok{fpc=}\OperatorTok{~}\NormalTok{Prob)}
\CommentTok{# estimador não-viciado da média de  salario e receita}
\NormalTok{sal_rec_aes_est <-}\StringTok{ }\KeywordTok{svymean}\NormalTok{(}\OperatorTok{~}\NormalTok{sal}\OperatorTok{+}\NormalTok{rec,popul_plan)}
\NormalTok{est_aes <-}\StringTok{ }\NormalTok{est_aes }\OperatorTok{+}\StringTok{ }\KeywordTok{coef}\NormalTok{(sal_rec_aes_est)}
\NormalTok{cov_mat_aes_est <-}\StringTok{ }\NormalTok{cov_mat_aes_est}\OperatorTok{+}\StringTok{ }\KeywordTok{attr}\NormalTok{(sal_rec_aes_est,}\StringTok{'var'}\NormalTok{)}
\NormalTok{\}}
\CommentTok{# 1-media de ybar; 2- média de ybar_w; 3- média de v0; 4- média de vhat_aes}

\NormalTok{mean_est_aas <-}\StringTok{ }\NormalTok{est_aas}\OperatorTok{/}\DecValTok{500}
\NormalTok{mean_est_aes <-}\StringTok{ }\NormalTok{est_aes}\OperatorTok{/}\DecValTok{500}
\NormalTok{mean_cov_mat_aas_est <-}\StringTok{ }\NormalTok{cov_mat_aas_est}\OperatorTok{/}\DecValTok{500}
\NormalTok{mean_cov_mat_aes_est <-}\StringTok{ }\NormalTok{cov_mat_aes_est}\OperatorTok{/}\DecValTok{500}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{sal_mean_epa_est <-}\StringTok{ }\NormalTok{mean_cov_mat_aes_est[}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{]}\OperatorTok{/}\NormalTok{mean_cov_mat_aas_est[}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{]}
\NormalTok{sal_mean_epa_est}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rec_mean_epa_est <-}\StringTok{ }\NormalTok{mean_cov_mat_aes_est[}\DecValTok{2}\NormalTok{,}\DecValTok{2}\NormalTok{]}\OperatorTok{/}\NormalTok{mean_cov_mat_aas_est[}\DecValTok{2}\NormalTok{,}\DecValTok{2}\NormalTok{]}
\NormalTok{rec_mean_epa_est}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{mat_cov_pop <-}\StringTok{ }\KeywordTok{by}\NormalTok{(popul_dat, popul_dat}\OperatorTok{$}\NormalTok{estrat, }\ControlFlowTok{function}\NormalTok{(t)}\KeywordTok{var}\NormalTok{(}\KeywordTok{cbind}\NormalTok{(t}\OperatorTok{$}\NormalTok{sal,t}\OperatorTok{$}\NormalTok{rec)) )}

\NormalTok{mat_cov_aleat_verd <-(Wh[}\DecValTok{1}\NormalTok{]}\OperatorTok{^}\DecValTok{2}\OperatorTok{*}\NormalTok{(}\DecValTok{1}\OperatorTok{-}\NormalTok{fh[}\DecValTok{1}\NormalTok{])}\OperatorTok{/}\NormalTok{nh[}\DecValTok{1}\NormalTok{])}\OperatorTok{*}\NormalTok{mat_cov_pop[[}\DecValTok{1}\NormalTok{]]}\OperatorTok{+}\NormalTok{(Wh[}\DecValTok{2}\NormalTok{]}\OperatorTok{^}\DecValTok{2}\OperatorTok{*}\NormalTok{(}\DecValTok{1}\OperatorTok{-}\NormalTok{fh[}\DecValTok{2}\NormalTok{])}\OperatorTok{/}\NormalTok{nh[}\DecValTok{2}\NormalTok{])}\OperatorTok{*}\NormalTok{mat_cov_pop[[}\DecValTok{2}\NormalTok{]]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tab_}\DecValTok{43}\NormalTok{ <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}
\StringTok{"Quantidade de interesse"}\NormalTok{ =}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Média Populacional"}\NormalTok{, }\StringTok{"Média de estimativas de média AAS"}\NormalTok{,}
  \StringTok{"Média de estimativas de média AES"}\NormalTok{),}
\NormalTok{Salá}\DataTypeTok{rios =} \KeywordTok{c}\NormalTok{(}\KeywordTok{mean}\NormalTok{(popul_dat}\OperatorTok{$}\NormalTok{sal), mean_est_aas[}\DecValTok{1}\NormalTok{], mean_est_aes[}\DecValTok{1}\NormalTok{] ),}
\DataTypeTok{Receitas =} \KeywordTok{c}\NormalTok{(}\KeywordTok{mean}\NormalTok{(popul_dat}\OperatorTok{$}\NormalTok{rec), mean_est_aas[}\DecValTok{2}\NormalTok{], mean_est_aes[}\DecValTok{2}\NormalTok{] )  }
\NormalTok{)}
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(tab_}\DecValTok{43}\NormalTok{, }\DataTypeTok{booktabs=}\OtherTok{TRUE}\NormalTok{ )    }
\end{Highlighting}
\end{Shaded}
...
\midrule
Média Populacional & 78.32826 & 2.106846\\
Média de estimativas de média AAS & 163.50334 & 4.171317\\
Média de estimativas de média AES & 78.06853 & 2.061913\\
\bottomrule
\end{tabular}
...
\StringTok{"Quantidade de interesse"}\NormalTok{ =}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Variância do estimador AES"}\NormalTok{, }\StringTok{"Média de estimativas de variância AES"}\NormalTok{,}
\StringTok{"Valor esperado AES do estimador AAS de variância"}\NormalTok{,}\StringTok{"Média de estimativas de variância AAS"}\NormalTok{),}
\NormalTok{Salá}\DataTypeTok{rios=} \KeywordTok{c}\NormalTok{(mat_cov_aleat_verd[}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{], mean_cov_mat_aes_est[}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{], }\OtherTok{NA}\NormalTok{, mean_cov_mat_aas_est[}\DecValTok{1}\NormalTok{,}\DecValTok{1}\NormalTok{]),}
\DataTypeTok{Receitas =} \KeywordTok{c}\NormalTok{(mat_cov_aleat_verd[}\DecValTok{2}\NormalTok{,}\DecValTok{2}\NormalTok{], mean_cov_mat_aes_est[}\DecValTok{2}\NormalTok{,}\DecValTok{2}\NormalTok{], }\OtherTok{NA}\NormalTok{, mean_cov_mat_aas_est[}\DecValTok{2}\NormalTok{,}\DecValTok{2}\NormalTok{])  }
\NormalTok{ )}
...
Quantidade.de.interesse & Salários & Receitas\\
\midrule
Variância do estimador AES & 244.1758 & 0.4349945\\
Média de estimativas de variância AES & 245.1878 & 0.4011822\\
Valor esperado AES do estimador AAS de variância & NA & NA\\
Média de estimativas de variância AAS & 1719.9794 & 1.2065052\\
\bottomrule
\end{tabular}

A seguir, reproduzimos os resultados do Exemplo \ref{exm:deffmult}, que
utiliza também os resultados da simulação acima.

Matriz de covariância dos estimadores ponderados da média, resultado em
\eqref{eq:epa7}:
...
\begin{verbatim}
##            [,1]      [,2]
## [1,] 244.175801 3.2357921
## [2,]   3.235792 0.4349945
\end{verbatim}

Média da matriz de covariância dos estimadores de média do salário e da
receita assumindo observações IID, resultado em \eqref{eq:epa8}:
...
\begin{verbatim}
##           [,1]      [,2]
## [1,] 1719.9794 26.780304
## [2,]   26.7803  1.206505
\end{verbatim}

Média da matriz de covariância dos estimadores de média do salário e da
receita assumindo observações AES, resultado em \eqref{eq:epa9}:
...
\begin{verbatim}
##            sal       rec
## sal 245.187815 3.1722720
## rec   3.172272 0.4011822
\end{verbatim}

A partir desses resultados, obtemos os efeitos multivariados para as
médias de Sálários e Receitas, em \eqref{eq:epa10}:
...
\KeywordTok{eigen}\NormalTok{(DELTA)}\OperatorTok{$}\NormalTok{values}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## [1] 0.4592617 0.1415845
\end{verbatim}

Os autovalores de DELTA 0.4592617 e 0.1415845 são os efeitos
