...
O erro padrão da razão entre razões estimada no exemplo anterior pode ser estimado por linearizão de Taylor, usando-se a função `svycontrast()` da library survey: 
