...
\BeginKnitrBlock{definition}<div class="definition"><span class="definition" id="def:unnamed-chunk-3"><strong>(\#def:unnamed-chunk-3) </strong></span>O **Efeito do Plano Amostral ($EPA$)**  **de Kish** para um estimador $\hat{\theta}$ é</div>\EndKnitrBlock{definition}
\begin{equation}
...
\BeginKnitrBlock{definition}<div class="definition"><span class="definition" id="def:unnamed-chunk-4"><strong>(\#def:unnamed-chunk-4) </strong></span>O efeito da especificação incorreta do plano amostral (ou do modelo) sobre a estimativa $v_{0}$ da variância do estimador $\hat{\theta}$ é </div>\EndKnitrBlock{definition}
...
![(\#fig:epacong)Valores de EPA e EPA de Kish para conglomeração](04-Cap4_files/figure-latex/epacong-1.pdf) 
