...
Utilizando o R, obtemos a seguir alguns resultados descritos no Exemplo \@ref(exm:deffuni).
...
Na simulação, usamos a library `sampling` [@R-sampling] para gerar amostras estratificadas de tamanho 30, com estratos definidos na Tabela \@ref(tab:estempr), para reproduzir os elementos das Tabelas \@ref(tab:proestmed)
e \@ref(tab:varest). 
...
```r
# muda escala de salários e receitas
...
sal_mean_epa_est <- mean_cov_mat_aes_est[1,1]/mean_cov_mat_aas_est[1,1]
sal_mean_epa_est
```
...
```r
rec_mean_epa_est <- mean_cov_mat_aes_est[2,2]/mean_cov_mat_aas_est[2,2]
rec_mean_epa_est
```
...
A seguir, reproduzimos a Tabela \@ref(tab:proestmed), usando os resultados da simulação:
...
Salários = c(mean(popul_dat$sal), mean_est_aas[1], mean_est_aes[1] ),
Receitas = c(mean(popul_dat$rec), mean_est_aas[2], mean_est_aes[2] )  
)
...
A seguir reproduzimos a Tabela \@ref(tab:varest), usando os resultados da simulação:


```r
tab_44 <- data.frame(
"Quantidade de interesse" = c("Variância do estimador AES", "Média de estimativas de variância AES",
"Valor esperado AES do estimador AAS de variância","Média de estimativas de variância AAS"),
Salários= c(mat_cov_aleat_verd[1,1], mean_cov_mat_aes_est[1,1], NA, mean_cov_mat_aas_est[1,1]),
Receitas = c(mat_cov_aleat_verd[2,2], mean_cov_mat_aes_est[2,2], NA, mean_cov_mat_aas_est[2,2])  
 )
...
A seguir, reproduzimos os resultados do Exemplo \@ref(exm:deffmult), que utiliza também os resultados da
simulação acima.


Matriz de covariância dos estimadores ponderados da média, resultado em \@ref(eq:epa7):
...
Média da matriz de covariância dos estimadores de média do salário e da receita assumindo observações IID, resultado em 
\@ref(eq:epa8):
...
Média da matriz de covariância dos estimadores de média do salário e da receita assumindo observações AES, resultado em 
\@ref(eq:epa9):
...
A partir desses resultados, obtemos os efeitos multivariados para as médias de Sálários e Receitas, em \@ref(eq:epa10):
...
```
Os autovalores de DELTA  0.4592617 e 
0.1415845 são os efeitos generalizados do plano amostral.
