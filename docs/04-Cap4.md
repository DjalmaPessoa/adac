...
```r
# cria matrizes para serem preenchidas
mat_sal_est<-matrix(NA,500,4)
mat_rec_est<-matrix(NA,500,4)
...
sal_med_aas_est<-mean(dados$sal)
rec_med_aas_est<-mean(dados$rec)
# estimador v0
...
sal_med_aes_est<-svymean(~sal,popul_plan) 
rec_med_aes_est<-svymean(~rec,popul_plan)
mat_sal_est[i,]<-c(sal_med_aas_est,coef(sal_med_aes_est),sal_var_aas,SE(sal_med_aes_est)^2)
mat_rec_est[i,]<-c(rec_med_aas_est,coef(rec_med_aes_est),rec_var_aas,SE(rec_med_aes_est)^2)
}
# 1-media de ybar; 2- média de ybar_w; 3- média de v0; 4- média de vhat_aes
sal_est_mean <-colMeans(mat_sal_est)
rec_est_mean <- colMeans(mat_rec_est)
# epa = média de vhat_aes/ média de v0

sal_mean_epa_est <- sal_est_mean[4]/sal_est_mean[3]
sal_mean_epa_est
```
...
```r
rec_mean_epa_est <- rec_est_mean[4]/rec_est_mean[3]
rec_mean_epa_est
```
...
vp_sal_h <- aggregate(sal ~ estrat, popul_dat, var)
vp_rec_h <- aggregate(rec ~ estrat, popul_dat, var)
...
Salários = c(mean(popul_dat$sal), sal_est_mean[1], sal_est_mean[2] ),
Receitas = c(mean(popul_dat$rec), rec_est_mean[1], rec_est_mean[2] )  
)
...
Salários= c(vp_sal, sal_est_mean[4], NA, sal_est_mean[3]),
Receitas = c(vp_rec, rec_est_mean[4], NA, rec_est_mean[3])  
 )
...
Resultados do Exemplo \@ref(exm:deffmult).
