...
Considerando os dados do Exemplo \@ref(exm:pnad) do Capítulo \@ref(modreg), vamos
...
\caption{(\#tab:unnamed-chunk-1)Proporções nas linha, desvios padrões e EPAs de `re` na população}
\centering
...
Vamos calcular, a título de ilustração, o efeito do plano amostral da estimativa na cela (1,1) da \@ref(tab:proplinha), apresentado na cela (1,7) dessa tabela.
...
\caption{(\#tab:unnamed-chunk-3)Proporções nas linha, desvios padrões e EPAs de sx na população}
\centering
