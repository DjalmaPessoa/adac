...
No Exemplo \@ref{ex:exebin} a estatística de teste $Z_{bin},$ que foi utilizada para comparar com um valor hipotético pré-fixado a proporção de empregados cobertos por plano de
...
coerência com a notação do Exemplo \@ref{ex:exebin}.
...
Para o plano amostral de conglomerados considerado no Exemplo \@ref{ex:exebin},
...
Table: (\#tab:testprop)Valores observados e valores-p de estatísticas de testes para os dados do Exemplo 4.4
...
\BeginKnitrBlock{example}<div class="example"><span class="example" id="exm:unnamed-chunk-6"><strong>(\#exm:unnamed-chunk-6) </strong></span>Teste de bondade de ajuste para a distribuição etária da PPV
...
Table: (\#tab:ppcount)Vetores de proporções por classes de idade da PPV 96/97 e Contagem
 96 e EPAs calculados para a PPV - Região Sudeste
