...
revela a Tabela \ref{table:Tab11}.
...
A Tabela \ref{table:Tab12} apresenta um resumo das distribuições dos pesos
...
são apresentados na Tabela \ref{table:Tab13}, juntamente com as medidas dos
efeitos de plano amostral (EPA). Exemplos de utilização da library `survey` para obtenção de estimativas apresentadas na \ref{table:Tab13} estão na Seção \@ref(epa). As outras estimativas da Tabela \ref{table:Tab13} podem ser obtidas de maneira análoga.
...
Como se pode observar da quarta coluna da Tabela \ref{table:Tab13}, os valores do
...
e definir variáveis de interesse por meio de transformação de variáveis existentes.
...
A seguir, mostramos a utilização da library survey do R para obter algumas estimativas da Tabela \ref{table:Tab13}. Vamos supor que os dados da pesquisa estão contidos no data frame `ppv1`, que contém as variáveis  que caracterizam o plano amostral
...
```
Para exemplificar, vamos estimar algumas características da população, descritas na Tabela \ref{table:Tab13}. Os totais das variáveis `analf1` e `analf2` para a região Sudeste fornecem os resultados nas linhas 4 e 5 da  Tabela \ref{table:Tab13}:
...
etárias consideradas, que fornece os resultados nas linhas 6 e 7 da Tabela \ref{table:Tab13}:
...
```
Observe que as estimativas de totais e desvios padrão obtidas coincidem com as Tabela \ref{table:Tab13}, porém as estimativas de Efeitos de Plano Amostral(DEff) são distintas.


### Estimativa do efeito de plano amostral (EPA)

Esse assunto será tratado em detalhes no Capítulo \@ref(epa) . Por enquanto, apresentaremos uma introdução necessária para compreender os valores na Tabela \ref{table:Tab13}.
...
Esses valores do EPA coincidem com os obtidos acima através da library survey e 
são distintos daqueles apresentados na Tabela \ref{table:Tab13}. Para obter os valores 
correspondentes aos da Tabela \ref{table:Tab13}, através da library survey, vamos definir as variáveis:
...
```
Leitura dos microdados usando a library `lodown`
...
as características do plano amostral da pesquisa. Isso é feito por meio da função `svydesign()` da library `survey. 
...
Utilizando a função `postStratify()` da library `survey` incorpora-se no objeto de desenho `pes_sel_des` as informações contidas no data frame `post_pop`.
...
Antes vamos definir uma nova variável `diag_dep` que é igual a 1 se recebe diagnóstico de depressão e zero caso contrário:
...
```{r,modclas, echo=FALSE, fig.cap="Modelagem Clássica"}
...
```{r, modamo,echo=FALSE, fig.cap="Amostragem Probabilística" }
...
```{r, modsup, echo=FALSE, fig.cap="Modelagem de Superpopulação"}
...
\ref{tab41} a título de ilustração. Note que a população
...
Os valores apresentados na Tabela \ref{tab41} para a RM do Rio de Janeiro são similares aos observados para as demais RMs, se consideradas as mesmas variáveis. Nota-se grande variação dos valores do EPA, cujos valores mínimo e máximo são de 1,28 e 111,27
...
valores na segunda linha da Tabela \ref{tab43}. Como previsto, observamos um
...
estimador as médias apresentadas na terceira linha da Tabela \ref{tab43}. Observamos ainda um pequeno vício da ordem de $-1,95\%$ e $-2,51\%$
...
população, resultando nos valores na linha 3 da Tabela \@ref{tab44},
e estimativas desses valores baseadas nas 500 amostras da simulação
são apresentadas na linha 4 da Tabela \@ref{tab44}. Os valores para o 
...
valores dos $\delta _{i}$ são distintos, os valores da Tabela \ref{tab47}
...
```{remark}
O plano amostral é também incorporado no método de estimação MPV através da expressão para a variância do total dos escores sob o plano amostral \@ref(eq:modpar14), onde as propriedades do plano amostral estão resumidas nas probabilidades de inclusão de primeira
...
calculadas pela função `svyglm`da library survey. 
...
```{r, testprop, echo=FALSE, eval=FALSE}
Valor_observado= round(c(x2p, x2pd,x2rv, x2rvd, x2w ),2)
...
testprop <- data.frame(Estatística = c("Pearson","Pearson ajustada","RV", "RV ajustada", "Wald"),
  gl = rep(1, 5), Valor_observado,valorp)
  

knitr::kable(testprop, booktabs=TRUE,
  caption = "Valores e valores-p de estatísticas de testes para os dados do Exemplo 4.4")
```
...
```{r, ppvcont , message=FALSE, warning=FALSE}
library(survey)
ppv1 <- readRDS("~\\GitHub\\adac\\data\\ppv.rds")
...
<!-- $\mathbf{P}_{0}/n=\frac{diag\left( \mathbf{p}_{0}\right) -\mathbf{p}_{0}\mathbf{p}_{0}^{^{\prime }}}{8.903}$, resultando em  -->
<!-- \[ -->
...
```{r, frqamost, eval=FALSE,echo=FALSE}
library(xtable)
pnadrj90 <- readRDS("~\\GitHub\\adac\\data\\pnadrj90.rds")
freq.sx.re <-table(pnadrj90$sx, pnadrj90$re , dnn= c("Sexo", "Renda"))
knitr::kable(xtable(freq.sx.re ,caption= "Frequências Amostrais por celas na PNADRJ90"))
```
...
```{r, proplinha, eval=FALSE, echo=FALSE}
library(survey)
library(pander)
pnadrj90<-transform(pnadrj90,sx=factor(sx),id=factor(id),ae=factor(ae),ht=factor(ht),re=factor(re))
pnad.des<-svydesign(id=~psu,strata=~stra,weights=~pesopes,data=pnadrj90,nest=TRUE)
marg_re <- as.data.frame(svyby(~re,~sx,pnad.des,svymean,keep.var=T,deff = TRUE))
marg_re <- transform (marg_re, se.re1= 10000*se.re1, se.re2= 10000*se.re2,se.re3= 10000*se.re3)
tmarg_re <- t(marg_re)
dimnames(tmarg_re)[[2]]<- c("sx_1","sx_2")

# pander(tmarg_re, digits=3, caption="Proporções nas linha, desvios padrões e EPAs de re para cada nivel de sx")
knitr::kable(tmarg_re[-1,] ,booktabs=TRUE, digits=3,
  caption="Proporções nas linha, desvios padrões e EPAs de re para cada nivel de sx")
```
```{r, eval=FALSE}
...
```{r, propcelas, echo=FALSE, eval=FALSE}
...
knitr::kable(frame_sx_re,booktabs=TRUE, digits=3,
...
Resultado dos testes obtidos pela survey são diferentes. Seriam incluídos?
...
de variâncias. O conjunto de programas pode ser adquirido do \emph{%
...
projetado e usado pelo \emph{US Bureau of the Census} para dados de pesquisa
...
aleatórios, de \emph{jackknife}, e de replicação balanceada. O
...
conglomerado primário combinado com a aproximação de lineariza\c{%
c}ão de Taylor para estimação de variâncias. Os dados devem
