...
## Impacto dos pesos da amostra da PPV
...
são apresentados na Tabela \ref{Tab13}, juntamente com as medidas dos
efeitos de plano amostral (EPA).

```{example}
Utilização da library survey do R para estimar totais e razões na PPV 
```
A seguir, mostramos a utilização da library survey do R para obter algumas estimativas da Tabela \@ref[Tab13].
Vamos supor que os dados da pesquisa estão contidos no data frame `ppv`, que contém as variáveis:
...
As outras estimativas da Tabela \@ref[Tab13] podem ser obtidas de maneira análoga.
...
Como se pode observar da quarta coluna da Tabela \@ref[b13], os valores do
...
discussão é apresentada no capítulo 2.

No terceiro capítulo apresentamos uma revisão sucinta, a título
...
No Capítulo 4 introduzimos o conceito de `Efeito do Plano Amostral
...
O Capítulo 6 trata da obtenção de `Estimadores de
