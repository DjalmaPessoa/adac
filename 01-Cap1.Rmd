...
```
Na library `survey`, uma alternativa para estimar por domínio é utililizar a função `svyby`. Poderíamos estimar os totais da variável `analf1` para as regiões `Nordeste (1)` e  `Sudeste(2)` da seguinte forma:
...
O data frame `pes_sel` contém as variáveis descritas no dicionário da PNS e algumas variáveis obtidas por transformação. Os comandos a seguir ilustram como seriam obtidas as novas variáveis a partir das variáveis no dicionário da PNS contidas em um data frame `x`. Os comandos não precisam ser executados, pois o data frame lido já contém essas variáveis.

```{r, eval=FALSE}
# x data frame só com as variáveis do dicionário
# Pessoas que se auto avaliam com saúde boa e muito boa
pes_sel <- transform( x , saude_b_mb = as.numeric( n001 %in% c( '1' , '2' ) ) )
# urbano / rural
pes_sel <- transform( x , situ = factor( substr( v0024 , 7 , 7 ) , labels = c( 'urbano' , 'rural' ) ) )

# sexo
pes_sel <- transform( x , c006 = factor( c006 , labels = c( 'masculino' , 'feminino' ) ) )
...
pes_sel <- transform( x , uf = factor( v0001 , labels = estado_names ) )

# região
pes_sel <- transform( x , region = factor( substr( v0001 , 1 , 1 ) , labels = c( "Norte" , "Nordeste" , "Sudeste" , "Sul" , "Centro-Oeste" ) ) )

# recodifica para numérico
pes_sel[ , c( 'p04101' , 'p04102' , 'p04301' , 'p04302' ) ] <- sapply( x[ , c( 'p04101' , 'p04102' , 'p04301' , 'p04302' ) ] , as.numeric )
# recodifica deslocamento para trabalho
pes_sel <-
	transform(
		x ,
...
pes_sel <- transform( x , tempo_desl = tempo_desl_trab + tempo_desl_athab )

pes_sel <- transform( x , atfi04 = as.numeric( tempo_desl >= 30 ) )


# idade categórica
pes_sel$age_cat <- factor( 1 + findInterval( as.numeric( x$c008 ) , c( 18 , 30 , 40 , 60 ) ) , labels = c( "0-17" , "18-29" , "30-39" , "40-59" , "60+" ) )

# race
pes_sel$raca <- as.numeric( pes_sel$c009 )
pes_sel[ pes_sel$raca == 9 , 'raca' ] <- NA
pes_sel$raca <- factor( pes_sel$raca , labels = c( 'Branca' , 'Preta' , 'Amarela' , 'Parda' , 'Indígena' ) )

# educação
pes_sel$educ <- factor( 1 + findInterval( as.numeric( pes_sel$vdd004 ) , c( 3 , 5 , 7 ) ) , labels = c( "SinstFundi" , "FundcMedi" , "MedcSupi" , "Supc" ) )

# número de pessoas no domicílio
pes_sel$c001 <- as.numeric(pes_sel$c001)
...
```{r, eval= FALSE}
saveRDS(pes_sel_des_pos,"C:/adac/PNS/2013 long questionnaire survey design.rds" )
```
...
Antes vamos definir uma nova variável `diag_dep` que é igual a 1 se recebe diagnóstico de depressão e zero caso contário:

```{r}
diag_dep <- as.numeric (pes_sel$q092=="1")
```

Vamos atualizar o objeto de desenho `pes_sel_des_pos` para incluir a variável `diad_dep` na componente `variables`:
...
Um intervalo de confiança aproximado de nível de confiança de 95% é dado por
...
dfsitusex <- data.frame(situ.sex = names(res)[1:4],prop = res[1:4], l.i.c = res[5:8], l.s.c=res[9:12] )
...
dfuf <- data.frame(uf = names(res)[1:27], prop = res[1:27], L.I.C. = res[28:54],L.S.C =res[55:81]  )
...
dfufsex <- data.frame (uf.sex =names(res)[1:54], prop = res[1: 54], L.I.C = res[55:108],L.S.C=res[109:162] )
