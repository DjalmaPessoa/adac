...
# leitura dos dados
library(anamco)
Nordeste <- ppv$pesof[ppv$regiao=="1"]
Sudeste <- ppv$pesof[ppv$regiao=="2"]
Q1 <- c(quantile(Nordeste,.25),quantile(Sudeste,.25), quantile(ppv$pesof,.25) )
names(Q1)<- NULL
Mediana<-  c(quantile(Nordeste,.50),quantile(Sudeste,.50), quantile(ppv$pesof,.50) )
names(Mediana)<- NULL
Q3 = c(quantile(Nordeste,.75),quantile(Sudeste,.75), quantile(ppv$pesof,.75))
names(Q3) <- NULL 
Pesos <- data.frame(
  Região = c("Nordeste", "Sudeste", "Nordeste_Sudeste"),
  Mínimo = c(min(Nordeste), min(Sudeste), min(ppv$pesof)),
  Q1 = Q1,
  Mediana= Mediana,
  Q3 = Q3,
  Máximo = c(max(Nordeste), max(Sudeste), max(ppv$pesof)) 
  )
...
knitr::kable(epas, booktabs=TRUE,align = "llll",
...
```{example, label= "exe12"}
Utilização da library survey do R para estimar alguns totais e razões na Tabela \@ref(tab:epas)
```
...
saveRDS(pnadc032016.calib, file="C:/adac/pnadc/pnadc_des.rds")
```

```{r, echo = FALSE}
pnadc032016.calib <- readRDS("C:/adac/pnadc/pnadc_des.rds")
```
...
```{r, echo=FALSE}
diagdepsex <- svyby(~diag_dep, ~c006, design = pes_sel.calib, svymean)
...
diagdepsitu <- svyby(~diag_dep, ~situ, design = pes_sel.calib, svymean)
...
diagdepsitusex <- svyby(~diag_dep, ~situ + c006, design = pes_sel.calib, svymean)
...
```{r, echo=FALSE, cache=TRUE}
diagdepuf <- svyby(~diag_dep, ~uf, design = pes_sel.calib, svymean)
...
diagdepufsex <- svyby(~diag_dep, ~uf + c006, design = pes_sel.calib, 
  svymean)
