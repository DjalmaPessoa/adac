...
Depois de baixar os dados, são criados os seuintes arquivos no diretório `C:/adac/PNS` :
...
```{remark}
Nos arquivos acima, além de as variáveis contidas no dicionário da PNS, foram acrescentadas variáveis derivadas, utilizadas nos exemplos no site [asdfree.com](https://github.com/ajdamico/asdfree/tree/master/Pesquisa%20Nacional%20De%20Saude) 
```
...
```{remark}
No data frame pes_sel os códigos das variáveis foram transformados para letras minúsculas.
```
...
O objeto de desenho `pes_sel_des` deve ser modificado para incorporar informações de totais disponíveis a partir do Censo Demográfico. Para isso, são definidos `post-pop`, definido a seguir.
...
As estimativas e os limites do intervalo de confiança aproximado com 95% de nível de confiança podem ser obtidos por meio da função:
