...
saveRDS(pnadc032016_calib, file="C:/adac/pnadc/pnadc_des.rds")
```

```{r, echo = FALSE}
pnadc032016_calib <- readRDS("C:/adac/pnadc/pnadc_des.rds")
```
...
Utilizando a função `postStratify()` da library `survey` [@R-survey] incorpora-se no objeto de desenho `pes_sel_plan` as informações contidas no data frame `post_pop`.


```{r}
pes_sel_calib <- postStratify( pes_sel_plan , ~v00293.y , post_pop )
```

Salvar objeto de desenho pós-estratificado para posterior utilização:

```{r, eval= FALSE}
saveRDS(pes_sel_calib, file = "C:/adac/PNS/pns_des_sel.rds" )
```

```{r, eval=FALSE}
pes_sel_calib <- readRDS("C:/adac/PNS/pns_des_sel.rds")
```

Os comandos acima foram apresentados apenas para ilustrar como, a partir dos microdados da pesquisa, obtemos o objeto de desenho da pesquisa. Não seria necessária a execução desses comandos pois o  objeto de desenho `pes_sel_calib` já se encontra disponível no arquivo 1 `2013 all questionnaire survey design.rds`. 
...
Como mencionado, o objeto de desenho pós-estratificado `pes_sel_calib` pode ser lido
do arquivo `2013 all questionnaire survey design.rds`. Vamos atualizar o objeto de desenho `pes_sel_calib` para incluir a variável `diad_dep` na componente `variables`:

```{r}
pes_sel_calib <- update (pes_sel_calib, diag_dep = diag_dep )
```
Para calcular a proporção de pessoas que receberam diagnóstico de depressão para
o país inteiro usamos o seguinte comando:

```{r}
diagdepbr <- svymean(~diag_dep, pes_sel_calib)
...
```{r, echo=FALSE}
diagdepsex <- svyby(~diag_dep, ~c006, design = pes_sel_calib, svymean)
...
diagdepsitu <- svyby(~diag_dep, ~situ, design = pes_sel_calib, svymean)
...
diagdepsitusex <- svyby(~diag_dep, ~situ + c006, design = pes_sel_calib, svymean)
...
```{r, echo=FALSE, cache=TRUE}
diagdepuf <- svyby(~diag_dep, ~uf, design = pes_sel_calib, svymean)
...
diagdepufsex <- svyby(~diag_dep, ~uf + c006, design = pes_sel_calib, 
  svymean)
...
pes_all_calib <- readRDS("C:/adac/PNS/2013 all questionnaire survey design.rds")
```
...
ou a variável `analf1` com o objeto `ppv_se_plan`. Porém na estimativa do denominador do EPA, obtida a partir da função `Vwaas`, 
