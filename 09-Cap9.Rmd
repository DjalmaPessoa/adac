...
forma são freq\"{u}entemente considerados na prática para
...
facilmente generalizados para o caso de mais de dois estratos, bastanto para
...
Adotando como referência básica \citeonline[cap.~11]{Sk89a}, vamos apresentar um resumo de alguns modelos hierárquicos
...
satisfazendo as condições em (\ref{9.5}). As equações (\ref
...
_{2j}$ (de nível 2), torna (\ref{9.11}) um modelo misto. Se os erros
fossem suprimidos em (\ref{9.10}), o modelo especificado só teria
efeitos fixos e a estimação dos parâmetros não traria
qualquer problema. Entretanto, a exclusão dos erros de nível 2 em (%
\ref{9.10}) não seria razoável, pois as variáveis definidas no
...
Além da parte fixa, o modelo contém uma \emph{parte residual, }com
os erros aleatórios com média zero: $\eta _{0j},$ que representa o
desvio da média dos indivíduos da turma $j$ com relação
à média total, e $\varepsilon _{ij},$ que é o desvio do $i$%
-ésimo aluno com relação à média da turma $j$. Vamos
...
A parte aleatória do modelo (\ref{9.12}) é o termo $\eta
_{0j}+\varepsilon _{ij}$, com distribuição tendo parâmetros $%
\sigma _{0}^{2}$ e $\sigma ^{2}$ a serem estimados.

O modelo (\ref{9.12}) não permite estudar interações entre
...
), verificamos que há uma correlação positiva entre respostas de
...
\sigma _{0}^{2}+\sigma ^{2}}.  (\#eq:hier15)
\end{equation}

A expressão (\ref{9.15}) define a \emph{correlação intraclasse }%
...
ESC_{ij}$ por turmas. Ela é a fração da variância residual atribuída à variância intra-conglomerado.
No caso de se supor adicionalmente a normalidade dos dados, foram propostos
vários métodos para estimação dos parâmetros do modelo (%
\ref{9.11}), entre os quais os métodos de Máxima Verossimilhanç%
a, de Máxima Verossimilhança Restrita e o Método Iterativo de
Mínimos Quadrados Generalizados. Detalhes destes métodos de estima\c{%
c}ão não serão abordados neste texto, e o leitor interessado
deve consultar, por exemplo, \citeonline{bryk}.
...
do plano a\-mos\-tral, tais como efeitos de estratificação e de
conglomeração, tornam o plano amostral ignorável,
condicionalmente nestas características, no sentido definido por
\citeonline{Rubin76}. Este raciocínio, porém, não é aplicável
...
à variável resposta. \citeonline{Pfefetalli} apresentam uma forma
...
```{example}
Plano amostral de pesquisa educacional (\citeauthoronline{lethonen}, \citeyear{lethonen}, p. 297).
```
...
plano amostral utilizado foi uma a\-mos\-tra estratificada de escolas
...
estimados, e o índice $j$ se refere a um domínio de estudo, $%
j=1,\ldots ,9$ (tais domínios não foram identificados na
referência citada).
...
chamaremos aqui de \emph{análise ingênua}. Neste caso, os
...
Outra opção é a \emph{análise agregada}, que incorpora o
...
seleção de modelos, com testes de siginificância baseados, por
...
No modelo hierárquico, para cada nível se considera uma variaç%
ão aleatória. Denotando por $p_{jk}$ a probabilidade de um
aluno da turma $k$ no domínio $j$ atingir o nível desejado em
Matemática, podemos modificar o modelo (\ref{9.16}) para incluir o
...
representa a variação aleatória no nível 2. A variaç%
ão no nível 1, entre alunos, é introduzida da seguinte forma.
...
```{example}
(\citeauthoronline{bryk}, \citeyear{bryk}, Cap. 5)
```

Os dados consitiram em respostas de $8.000$ professores aninhados em $357$
...
dos professores por escola) explicada pelas variáveis $\left(
x_{1},\ldots ,x_{6}\right) $ aumentou para
\[
\frac{0,084-0,055}{0,084}=35.
\]

Embora esse aumento do poder explicativo do modelo já tenha sido
subs\-tan\-cial, ainda é relativamente baixa a proporção de varia%
ção explicada, e portanto consideramos um terceiro modelo, em que
...
sugerindo que essa variável\emph{\ }é importante para explicar a
...
Agora vamos ver o que teria ocorrido caso um analista procurasse ajus\-tar
...
x_{1j},\ldots ,x_{7j}\right) $ da escola, que teriam seus valores
...
A introdução da variável $Comunit$\emph{, }neste modelo, só
aumentou a quantidade de variação explicada em $2,5\%$. A julgar por
este resultado, a importância da variável $Comunit$\emph{\ }é
...
hierárquico correspondente (Modelo III) este denominador é $\tau
_{00},$ que é a parte explicável da variação. A
...
\allowbreak pes\-qui\-sas complexas, indicando em cada caso como a
...
de \citeonline{bryk}, denominada ''Pontos básicos sobre efeitos
...
decorre da amostragem por conglomerado.\medskip
...
este fen\^{o}meno seja, freq\"{u}entemente, considerado como de
...
fatores organizacionais.\medskip
...
suas implicações para a modelagem desagregada.\medskip
...
pelos pacotes padrões até recentemente, e freq\"{u}entemente
...
planejamento amostral, tais como estratificação, conglomeraç%
ão e probabilidades desiguais de inclusão, a situação
desejável é incorporar na formulação do modelo as informa\c{c%
}ões necessárias para que o plano amostral seja ignorável na
...
precisam ser incorporados para ajustar o modelo (veja \citeauthoronline{Pfefetalli}, \citeyear{Pfefetalli}.
...
pode apresentar desafios não triviais ao analista: a especificaç%
ão detalhada da forma do modelo, a estimação de seus
inúmeros parâmetros dada apenas uma amostra das unidades da popula\c{%
c}ão, a interpretação das estimativas dos parâmetros e o
diagnóstico do ajuste efetuado podem todas se tornar tarefas bastante
complexas. Essa dificuldade é também mencionada por \citeonline[p.~9]{Sk89a}.
...
a\-mos\-trais por analistas secundários é geralmente realizada em
...
de microdados nos quais, entretanto, as informações de identifica\c{c%
}ão do plano amostral são omitidas (total ou parcialmente) para
...
impossível. Para citar um exemplo, numa pes\-quisa amostral conglomerada
