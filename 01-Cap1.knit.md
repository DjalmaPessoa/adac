...
ppv_plan <- svydesign(data = ppv_dat, strata = ~estratof, ids = ~nsetor, 
                      nest = TRUE, weights = ~pesof)
```

Como todos os exemplos a seguir serão relativos a estimativas na Região Sudeste,
vamos criar um objeto de desenho restrito a essa região.-
