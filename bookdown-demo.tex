...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{source}\NormalTok{(}\StringTok{"C:}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{Users}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{anthonyd}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{Documents}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{GitHub}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{adac}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{data}\CharTok{\textbackslash{}\textbackslash{}}\StringTok{ppv1.R"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\section{Impacto dos pesos da amostra da
PPV}\label{impacto-dos-pesos-da-amostra-da-ppv}
...
\begin{longtable}[]{@{}lll@{}}
\toprule
Estrato Geográfico & População & Amostra\tabularnewline
\midrule
\endhead
1 RM Fortaleza & 2.263 & 62\tabularnewline
2-RM Recife & 2.309 & 61\tabularnewline
3-RM Salvador & 2.186 & 61\tabularnewline
4-Restante Nordeste Urbano & 15.057 & 61\tabularnewline
5-Restante Nordeste Rural & 23.711 & 33\tabularnewline
6-RM Belo Horizonte & 3.283 & 62\tabularnewline
7-RM Rio de Janeiro & 10.420 & 61\tabularnewline
8-RM São Paulo & 14.943 & 61\tabularnewline
9-Restante Sudeste Urbano & 25.855 & 61\tabularnewline
10-Restante Sudeste Rural & 12.001 & 31\tabularnewline
Total & 112.001 & 554\tabularnewline
\bottomrule
\end{longtable}
...
A Tabela \ref{Tab12} apresenta um resumo das distribuições dos pesos
amostrais para as Regiões Nordeste (5 estratos geográficos) e Sudeste (5
estratos geográficos) separadamente e para o conjunto da amostra da PPV.
...
para cada uma das estimativas consideradas usando a
\texttt{library\ survey} do R . Essa razão fornece uma medida do efeito
de ignorar o plano amostral . Os resultados das estimativas ponderadas e
variâncias considerando o plano amostral são apresentados na Tabela
\ref{Tab13}, juntamente com as medidas dos efeitos de plano amostral
(EPA).

\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-4}{}{\label{ex:unnamed-chunk-4}}Utilização
da library survey do R para estimar totais e razões na PPV
\EndKnitrBlock{example} A seguir, mostramos a utilização da library
survey do R para obter algumas estimativas da Tabela @ref{[}Tab13{]}.
Vamos supor que os dados da pesquisa estão contidos no data frame
\texttt{ppv}, que contém as variáveis:
...
\item
  ANALF1.SE - indicador de analfabeto na faixa etária de 7 a 14 anos na
  Região Sudeste;
\item
  ANALF2.SE - indicador de analfabeto na faixa etária acima de 14 anos
  na Região Sudeste;
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# library(adacr)}
\CommentTok{# data(ppv1)}
\NormalTok{ppv1 <-}\StringTok{ }\KeywordTok{transform} \NormalTok{( ppv1, }
\DataTypeTok{ANALF1.SE =} \NormalTok{(V04A01 ==}\StringTok{ }\DecValTok{2} \NormalTok{|}\StringTok{ }\NormalTok{V04A02 ==}\StringTok{ }\DecValTok{2}\NormalTok{) &}\StringTok{ }\NormalTok{(V02A08 >=}\StringTok{ }\DecValTok{7} \NormalTok{&}\StringTok{ }\NormalTok{V02A08 <=}\StringTok{ }\DecValTok{14}\NormalTok{) &}\StringTok{ }\NormalTok{REGIAO ==}\DecValTok{2}\NormalTok{,}
\DataTypeTok{ANALF2.SE =} \NormalTok{((V04A01 ==}\StringTok{ }\DecValTok{2} \NormalTok{|}\StringTok{ }\NormalTok{V04A02 ==}\StringTok{ }\DecValTok{2}\NormalTok{) &}\StringTok{ }\NormalTok{(V02A08 >}\DecValTok{14}\NormalTok{)) &}\StringTok{ }\NormalTok{REGIAO ==}\DecValTok{2}\NormalTok{,}
\DataTypeTok{FAIXA1.SE =} \NormalTok{(V02A08 >=}\StringTok{ }\DecValTok{7} \NormalTok{&}\StringTok{ }\NormalTok{V02A08 <=}\StringTok{ }\DecValTok{14}\NormalTok{) &}\StringTok{ }\NormalTok{REGIAO ==}\DecValTok{2}\NormalTok{, }
\DataTypeTok{FAIXA2.SE =} \NormalTok{(V02A08 >}\StringTok{ }\DecValTok{14}\NormalTok{) &}\StringTok{ }\NormalTok{REGIAO ==}\StringTok{ }\DecValTok{2} \NormalTok{) }
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{svytotal}\NormalTok{(~ANALF1.SE, ppv.des, }\DataTypeTok{deff =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                    total        SE     DEff
## ANALF1.SEFALSE 102466391   1698198 465.2732
## ANALF1.SETRUE    1174220    127982   2.6426
\end{verbatim}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{svytotal}\NormalTok{(~ANALF2.SE, ppv.des, }\DataTypeTok{deff =} \OtherTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##                   total       SE     DEff
## ANALF2.SEFALSE 98848267  1680091 115.6674
## ANALF2.SETRUE   4792344   318877   4.1667
\end{verbatim}

Em seguida, estimaremos a proporção de analfabetos na faixa etária de 7
a 14 anos:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ANALF1.SE_FAIXA1.SE_raz<-}\KeywordTok{svyratio}\NormalTok{(~ANALF1.SE,~FAIXA1.SE, ppv.des,}\DataTypeTok{deff=}\NormalTok{T)}
\KeywordTok{coef}\NormalTok{(ANALF1.SE_FAIXA1.SE_raz) }\CommentTok{# razão}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{SE}\NormalTok{(ANALF1.SE_FAIXA1.SE_raz) }\CommentTok{# se da razão}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{deff}\NormalTok{(ANALF1.SE_FAIXA1.SE_raz) }\CommentTok{# EPA da razão}
\end{Highlighting}
\end{Shaded}
...
As outras estimativas da Tabela @ref{[}Tab13{]} podem ser obtidas de
maneira análoga.
...
Como se pode observar da quarta coluna da Tabela @ref{[}b13{]}, os
...
Tabela \ref{Tab13} não foram escolhidas de forma a acentuar os efeitos
ilustrados, mas tão somente para representar distintos parâmetros
(médias, razões, totais, proporções) e variáveis de interesse. Os
resultados apresentados para as estimativas de EPA ilustram bem o
cenário típico em pesquisas amostrais complexas: o impacto do plano
amostral sobre a inferência varia conforme a variável e o tipo de
parâmetro de interesse. Note ainda que à exceção do menor valor, todas
as demais estimativas de EPA apresentaram valores superiores a 2.

\section{Objetivos do Livro}

Este livro tem três objetivos principais:

\begin{enumerate}
\item[1)]  \textbf{ilustrar e analisar o impacto das simplificações
feitas ao utilizar pacotes usuais de análise de dados quando estes são provenientes
de pesquisas amostrais complexas};

\item[2)]  \textbf{apresentar uma coleção de métodos e recursos computacionais 
disponíveis para análise de dados amostrais complexos, equipando o analista para trabalhar 
com tais dados, reduzindo assim o risco de inferências incorretas};

\item[3)]  \textbf{ilustrar o potencial analítico de muitas das pesquisas produzidas 
por agências de estatísticas oficiais para responder questões de interesse, mediante 
uso de ferramentas de análise estatística agora já bastante difundidas, aumentando 
assim o valor adicionado destas pesquisas}.
...
dessa discussão é apresentada no capítulo 2.

No terceiro capítulo apresentamos uma revisão sucinta, a título de
...
No Capítulo 4 introduzimos o conceito de
...
No Capítulo 5 estudamos a questão do uso de pesos ao analisar dados
...
O Capítulo 6 trata da obtenção de
...
Os capítulos sétimo e oitavo tratam da análise de dados categóricos com
ênfase na adaptação dos testes clássicos para proporções, de
independência e de homogeneidade em tabelas de contingência, para dados
provenientes de pesquisas amostrais complexas. Apresentamos correções
das estatísticas clássicas e a estatística de Wald baseada no plano
amostral. As referências básicas usadas nesses capítulos foram os livros
cap. 4, \citep{SHS89} e cap. 7 \citep{lethonen}. Também são apresentadas
as idéias básicas de como efetuar ajuste de modelos log-lineares a dados
de freqüências em tabelas de múltiplas entradas.
...
O capítulo treze discute algumas formas alternativas de analisar dados
de pesquisas complexas, contrapondo algumas abordagens distintas à que
demos preferência nos capítulos anteriores, para dar aos leitores
condições de apreciar de forma crítica o material apresentado no
restante deste livro. Entre as abordagens discutidas, há duas
...
\texttt{análise\ agregada}, e procura `'eliminar'`da análise efeitos
tais como conglomeração induzida pelo plano amostral, considerando tais
efeitos como'`ruídos'`ou fatores de perturbação que'`atrapalham'' o
emprego dos procedimentos clássicos de estimação, ajuste de modelos e
teste de hipóteses.
...
revisão dos pacotes procura também apresentar suas características mais
importantes. Vários destes programas podem ser adquiridos gratuitamente
via \texttt{internet}, nos endereços fornecidos de seus produtores. Com
isto pretendemos indicar aos leitores o caminho mais curto para permitir
a implementação prática das técnicas e métodos aqui discutidos.
...
\section{Modelagem - Primeiras
Idéias}\label{modelagem---primeiras-ideias}
...
apresentada na Figura \ref{fig:Figura21} a seguir, e uma descrição
esquemática resumida é apresentada na Tabela \ref{Tab21}.

\begin{figure}[htbp]
\centering
\includegraphics{Figuras/Figura21Nova.pdf}
\caption{Fig 2.1: Modelagem Clássica}
\end{figure}
...
Vários exemplos discutidos ao longo dos capítulos seis, sete e oito
ilustram situações deste tipo. No capítulo nove o foco vai ser a
estimação não paramétrica da forma da função \(f(y;\theta )\).
...
atividade e outras. Rodrigues(2003) examinou este problema empregando
...
\begin{figure}[htbp]
\centering
\includegraphics{Figuras/figura22borda.pdf}
\caption{Fig 2.2: Amostragem Probabilística}
\end{figure}
...
Capítulo 1. Os dados obtidos são utilizados principalmente para
descrição da população finita, sendo calculadas estimativas de totais,
médias, razões, etc. Sob essa abordagem, os pontos i) a iv) do Capítulo
1 são devidamente considerados tanto na estimação de parâmetros
descritivos desses tipos, como também na estimação de variâncias dos
estimadores.
...
incorpora na análise os pontos i) a iii) do Capítulo 1 mediante
aproveitamento da estrutura do planejamento amostral como na
\texttt{abordagem\ 2}.

\subsection{Abordagem 3 - Modelagem de
Superpopulação}\label{abordagem-3---modelagem-de-superpopulacao}
...
Veja uma representação gráfica resumida desta abordagem na Figura 2.3.

\begin{figure}[htbp]
\centering
\includegraphics{Figuras/figura23borda.pdf}
\caption{Fig 2.3: Modelagem de Superpopulação}
\end{figure}
...
A título de recordação, serão apresentados no Capítulo 3 alguns
resultados básicos da Teoria de Amostragem. A ênfase do texto, porém,
será na apresentação da \texttt{abordagem\ 3}, sendo para isto
...
\texttt{complexas\textquotesingle{}\textquotesingle{},\ tais\ como\ as\ executadas\ pelo\ IBGE,\ o\ usuário\ precisa\ incorporar\ as\ informações\ sobre\ pesos\ e\ planos\ amostrais\ utilizados.\ Em\ geral,\ ao\ publicar\ os\ resultados\ das\ pesquisas,\ os\ pesos\ são\ considerados,\ sendo\ possível\ produzir\ estimativas\ pontuais}corretas''
utilizando os pacotes tradicionais. Por outro lado, para construir
intervalos de confiança e testar hipóteses sobre parâmetros de modelos,
seria preciso o conhecimento das estimativas de variâncias e
covariâncias das estimativas, obtidas a partir do plano amostral
utilizado. Mesmo conhecendo o plano amostral, geralmente não é simples
incorporar pesos e plano amostral na análise sem o uso de pacotes
especializados, ou de rotinas específicas já agora disponíveis em alguns
dos pacotes mais comumente utilizados (por exemplo, SAS, Stata, SPSS, ou
R entre outros). Tais pacotes especializados ou rotinas específicas
utilizam na maioria métodos aproximados para estimar matrizes de
covariância, tais como os de Máxima Pseudo-Verossimilhança e de
Linearização, que serão descritos mais adiante.
...
de muitos remédios, pode ter contra-indicações. Cabe ao usuário ``ler a
bula'' e identificar situações em que o uso de tais pacotes pode ser
inadequado, e buscar opções de rotinas específicas ou de pacotes
especializados capazes de incorporar adequadamente a estrutura do plano
amostral nas análises. Ao longo deste livro faremos uso intensivo da
biblioteca {[}\citet{R-survey}\} disponível no pacote \texttt{R}, mas o
leitor encontrará funcionalidade semelhante em vários outros pacotes.
Nossa escolha se deveu a dois fatores principais: primeiro ao fato do
pacote R ser aberto, livre e gratuito, dispensando o usuário de custos
de licenciamento, bem como possibilitando aos interessados o acesso ao
...
transitória. No presente momento, a biblioteca \emph{survey} é a coleção
...
pacotes com interfaces gráficas do tipo ``aponte e clique'' terão
...
\section{Planejamento Amostral}\label{planejamento-amostral}
...
\section{Planos Amostrais Informativos e
Ignoráveis}\label{planos-amostrais-informativos-e-ignoraveis}
...
abordagem clássica descrita na Seção \ref{classic} pode prosseguir sem
...
pode viciar a inferência. Veja o Exemplo \ref{Ex2.1} adiante.
...
literatura - veja por exemplo \citeonline{Sugden84} ou os capítulos 1 e
2 de \citeonline{CHSK2003}. Porém testar a ignorabilidade do plano
amostral é muitas vezes complicado. Em caso de dificuldade, o uso de
pesos tem papel fundamental.
...
incorporar pesos adequados na análise, como se verá no Capítulo 3. Essa
forma porém, não resolve por si só o problema de estimação da precisão
das estimativas pontuais, nem mesmo o caso da estimação pontual de
parâmetros em modelos de superpopulação, o que vai requerer métodos
específicos discutidos no Capítulo 5.
...
\textbf{Ex2.2} Efeito da amostragem estratificada simples com alocação
desproporcional
...
\chapter{Estimação Baseada no Plano
Amostral}\label{estimacao-baseada-no-plano-amostral}

\section{Estimação de Totais}\label{estimacao-de-totais}
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-8}{}{\label{ex:unnamed-chunk-8}}Amostragem
Aleatória Simples Sem Reposição (AAS)
...
desvios padrões ou mesmo de coeficientes de varia ção) é requerida para
...
discussão no Capítulo 10).

\section{\texorpdfstring{Linearização de Taylor para Estimar variâncias
\label{taylor}}{Linearização de Taylor para Estimar variâncias }}\label{linearizacao-de-taylor-para-estimar-variancias}
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-9}{}{\label{ex:unnamed-chunk-9}}Matriz
de covariância para um vetor de razões
\EndKnitrBlock{example}
...
totais, pode ser aplicado também para estimar (por linearizaç\% ão)
quantidades populacionais que possam ser representadas como funç\% ões
de totais, conforme discutido na Seção \ref{taylor}. De fato, esse
método fornece a base para vários dos pacotes estatísticos
especializados em cálculo de variâncias considerando o plano amostral,
tais como SUDAAN, CENVAR, STATA ou PC-CARP (veja discussão no Capítulo
10).
...
\widehat{\theta }_{g} 
\] onde \(\widehat{\theta }_{g}\) é um estimador de \$\theta \$ obtido
da amostra após eliminar os elementos do grupo \(g\), empregando a mesma
forma funcional adotada no cálculo do estimador \(\widehat{\theta }\)
que considera a amostra inteira. A estimação da variância por esse
método pode então ser feita de duas maneiras alternativas, usando um dos
...
é um estimador pontual \emph{jackknife} para \(% \theta ,\) alternativo
ao estimador da amostra inteira \(\hat{\theta}\).

\textbf{Observação:} A descrição do método \emph{jackknife }aqui
apresentada não cobre o caso de planos amostrais estratificados, que é
mais complexo. Para detalhes sobre este caso, consulte
\citeonline[p. 174]{W85}.

\textbf{Observação:} O estimador
\(\widehat{V}_{J2}\left( \widehat{\theta }\right)\) é maisconservador
que o estimador \(\widehat{V}_{J1}\left( \widehat{\theta }\right)\).

\textbf{Observação:} é comum aplicar a técnica fazendo o número de
...
\protect\hypertarget{def:unnamed-chunk-10}{}{\label{def:unnamed-chunk-10}}O
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-11}{}{\label{ex:unnamed-chunk-11}}\label{epakish}Efeitos
de plano amostral de Kish para estimadores de totais com amostragem
conglomerada em dois estágios.
\EndKnitrBlock{example}
...
variáveis são apresentados na Tabela \ref
{tab41} a título de ilustração. Note que a população alvo considera
apenas moradores em domicílios particulares permanentes na Região
...
O que se observou no Exemplo \ref{epakish} com respeito à dificuldade de
uso dos EPAs de Kish calculados para fins analíticos também se aplica
para outras situações e é uma deficiência estrutural do conceito de EPA
proposto por Kish. Para tentar contornar essa dificuldade, é necessário
considerar um conceito ampliado de EPA, correspondente ao conceito de
\emph{misspecification effect} \texttt{meff} proposto por p.~24,
\citep{SHS89}, que apresentamos e discutimos nesta seção.
...
\BeginKnitrBlock{definition}
\protect\hypertarget{def:unnamed-chunk-12}{}{\label{def:unnamed-chunk-12}}O
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-13}{}{\label{ex:unnamed-chunk-13}}\label{deffuni}Efeitos
de plano amostral para estimação de médias na amostragem estratificada
simples com alocação desproporcional
\EndKnitrBlock{example}
...
calculados, obtendo-se os valores na primeira linha da
Tabela.\ref{tab43}.
...
população, resultando nos valores na linha 3 da Tabela \ref{tab44}, e
estimativas desses valores baseadas nas 500 amostras da simulação são
apresentadas na linha 4 da Tabela \ref{tab44}. Os valores para o
...
\textbackslash{}begin\{center\} \textbackslash{}begin\{table\}{[}tbp{]}
\centering
...
\textbf{Observação:} Neste exemplo não foi feito uso analítico dos dados
e sim descritivo, onde é usual incorporar os pesos no cálculo de
estimativas e variâncias. Não seria esperado usar um estimador ponderado
para a média e não considerar os pesos no cálculo de variâncias, como
fizemos neste exemplo.

\textbf{Observação:} O exemplo mostra que ignorar a estratificação ao
calcular \(v_{0}\) diminui o \(\mathbf{EPA}\).
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-14}{}{\label{ex:unnamed-chunk-14}}População
conglomerada com conglomerados de tamanho 2 \citep{SHS89}, p.~25
\EndKnitrBlock{example}
...
\citeonline[cap. 2]{SilvaMou} e \citeonline{haggard}. Suponha que um
único par é sorteado ao acaso da população e que os valores \(y_{1}\) e
\(y_{2}\) são observados para as duas unidades do par selecionado. O
modelo assumido pode então ser representado como \[
\left\{ 
\begin{array}{l}
...
A Figura 4.1 plota os valores de
...
\begin{figure}[htbp]
\centering
\includegraphics{Figuras/figura41borda.pdf}
\caption{Fig 2.1: Valores de EPA e EPA de Kish para conglomeração}
\end{figure}
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-15}{}{\label{ex:unnamed-chunk-15}}\label{exebin}Teste
de hipótese sobre proporção
...
\emph{tamanho efetivo} da amostra é \(\overline{n}=50\) , em contraste
com o valor \(1.000\) usado no teste anterior. O termo
...
conceito, vamos reconsiderar o Exemplo \ref{deffuni} de estimação de
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-16}{}{\label{ex:unnamed-chunk-16}}\label{deffmult}Efeitos
...
R\$ \(1.000.000\)) definidas na população de empresas do Exemplo \ref
{deffuni} e calcular a matriz
...
notação do Exemplo \ref{deffuni}, temos que \[
...
Exemplo \ref{deffuni} e coincidem, respectivamente, com os valores
...
feito no Exemplo \ref{deffuni}. Para isto, foram utilizadas outras
\(500\) amostras de tamanho \(60\) segundo o plano amostral descrito no
Exemplo \ref{deffuni}. Para cada uma das \(500\) amostras foram
...
Os autovalores \(\delta _{1}=0,447\) e \(\delta _{2}=0,139\) de
\$\mathbf{\Delta } \$ fornecem os efeitos generalizados do plano
amostral.
...
diferença entre estes \(p\)valores. Consideremos a região crítica do
...
\(\delta _{1}\) no lugar de \$\delta  \$.

\textbackslash{}begin\{center\} \textbackslash{}begin\{table\}{[}tbp{]}
\centering
\textbackslash{}caption\{Níveis de significância (\(\%\)) verdadeiros do
teste \(T^{2}\) para o nível nominal de \(5\%\) assumindo autovalores
iguais para \$\mathbf{\Delta}\} \label{tab47}
...
\textbackslash{}end\{table\} \textbackslash{}end\{center\}

\chapter{Ajuste de Modelos
Paramétricos}\label{ajuste-de-modelos-parametricos}

\section{Introdução (\#modpar1)}\label{introducao-modpar1}
...
valores das variáveis de interesse nas populações finitas, tais como
...
Os exemplos apresentados no Capítulo 4 demonstram claramente a inadequa
...
Embora as medidas propostas no Capítulo 4 para os efeitos de plano
amostral sirvam para avaliar o impacto de ignorar o plano amostral nas
inferências descritivas ou mesmo analíticas baseadas em dados amostrais,
elas não resolvem o problema de como incorporar o plano amostral nessas
análises. No caso das inferências descritivas usuais para médias, totais
e proporções, o assunto é amplamente tratado na literatura de amostragem
e o interessado em maiores detalhes pode consultar livros clássicos como
\citep{cochran}, ou mais recentes como \citep{SSW92}. Já os métodos
requeridos para inferências analíticas só recentemente foram
consolidados em livro (\citep{SHS89}). Este capítulo apresenta um dos
métodos centrais disponíveis para ajuste de modelos paramétricos
regulares considerando dados amostrais complexos, baseado no trabalho de
\citep{binder87}. Antes de descrever esse método, entretanto, fazemos
breve discussão sobre o papel dos pesos na análise de dados amostrais,
considerando o trabalho de \citep{Pfeff}.
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-17}{}{\label{ex:unnamed-chunk-17}}Uso
...
\ref{tab51} revela como variam essas probabilidades de seleção entre as
regiões cobertas pela amostra da PNAD de 93. Como se pode observar, tais
probabilidades de inclusão chegam a ser \(5\) vezes maiores em Belém do
que em São Paulo, e portanto variação semelhante será observada nos
pesos.
...
(tal como definidos por @ref(3.17), com pesos dados por
\(w_{i}^{R}=\pi_{i}^{-1}X\,/\,\widehat{X}_{\pi }\) (veja
\eqref{eq:modpar19}, onde \(X\) é o total da população no estrato obtido
...
exposta (veja \citeonline[cap. 6]{Silva}).
...
texto será o método de \emph{Máxima Pseudo-Verossimilhança}, que
...
e de log-verossimilhança \emph{populacionais} seriam dadas
respectivamente por
...
As equações de verossimilhança \emph{populacionais} correspondentes são
dadas por
...
Sob condições de regularidade (\citeonline[p. 281]{cox}), a solução
\(\mathbf{\theta }_{U}\) deste sistema é o \emph{Estimador de
Máxima Verossimilhança}\textbf{\ }de \(\mathbf{\theta }\) no caso de um
\textbf{\ }\emph{censo}. Podemos considerar \(\mathbf{\theta }_{U}\)
como uma \emph{Quantidade Descritiva Populacional Correspondente (QDPC)}
a \(\mathbf{\theta }\), no sentido definido por \citeonline{Pfeff},
sobre a qual se deseja fazer inferências com base em informações da
amostra. Essa definição da QDPC \(\mathbf{\theta }_{U}\) pode ser
generalizada para contemplar outras abordagens de inferência além da
abordagem clássica baseada em maximização da verossimilhança. Basta para
isso especificar outra regra ou critério a otimizar e então definir a
QDPC como a solução ótima segundo essa nova regra. Tal generalização,
discutida em \citeonline{Pfeff}, não será aqui considerada para manter a
simplicidade.
...
constituir-se num \emph{pseudo-parâmetro}, eleito como alvo da
...
estimador adequado para \$ \mathbf{\theta }\_\{U\}\$ será geralmente
adequado também para \(\mathbf{ \theta }\).
...
(veja capítulo 3) onde \(w_{i}\) são pesos propriamente definidos. Com
essa notação, podemos agora obter um estimador para
\(\mathbf{\theta }_{U}\) resolvendo o sistema de equações obtido
igualando o estimador \(\mathbf{\hat{T}}\) do total \(\mathbf{T}\) a
zero.\medskip

\BeginKnitrBlock{definition}
\protect\hypertarget{def:unnamed-chunk-18}{}{\label{def:unnamed-chunk-18}}O
estimador de Máxima Pseudo-Verossimilhança (MPV) \linebreak \$
\mathbf{\hat{\theta}}\_\{MPV\}\$ de \(\mathbf{\theta }_{U}\) (e
consequentemente de \(\mathbf{\theta }\)) será a solução das equações de
Pseudo-Verossimilhança dadas por
...
{taylor}) e considerando os resultados de \citeonline{binder83}, podemos
obter a variância de aleatorização assintótica do estimador
...
\$V\_\{p\}\left[\sum_{i\in s}w_{i}\mathbf{u}_{i}\left( \mathbf{\theta}_{U}\right) \right] \$
é a matriz de variância (de aleatorização) do estimador do total
populacional dos escores e
\(\hat{V}_{p}\left[\sum_{i\ins}w_{i}\mathbf{u}_{i}\left( \mathbf{\hat{\theta}}_{MPV}\right)\right]\)
...
de regularidade enunciadas em \citeonline[apêndice] e revistas em
...
usados são os do estimador \$\pi \$-ponderado ou de Horvitz-Thompson
para totais, dados pelo inverso das probabilidades de inclusão dos
...
\(\hat{V}_{p}\left( \mathbf{\hat{\theta}}_{\pi }\right)\) para calcular
regiões ou intervalos de confiança e/ou estatísticas de teste baseadas
na distribuição normal e utilizá-las para fazer inferência sobre os
componentes de \(\mathbf{\theta}\). \textbackslash{}end\{enumerate\}

\textbf{Observação:} No Método de Máxima Pseudo-Verossimilhança, os
pesos amostrais são incorporados na análise através das equações de
estimação dos parâmetros \eqref{eq:modpar7} e através das equaç ões de
estimação da matriz de covariância dos estimadores
\eqref{eq:modpar13}-\eqref{eq:modpar15}.

\textbf{Observação:} O plano amostral é também incorporado no método de
estimaç ão MPV através da expressão para a variância do total dos
escores sob o plano amostral \eqref{eq:modpar14}, onde as propriedades do
plano amostral estão resumidas nas probabilidades de inclusão de
primeira e segunda ordem, isto é, os \(\pi _{i}\) e os \(\pi _{ij}\)
respectivamente.

\textbf{Observação:} Sob probabilidades de seleção iguais, os pesos
\(\pi _{i}^{-1}\) serão constantes e o estimador pontual
\(\hat{\theta}_{\pi }\) será idêntico ao estimador de Máxima
Verossimilhança (MV) ordinário para uma amostra de observações IID com
dis-tribui\c{c
}ão \(f\left(\mathbf{y;\theta }\right)\). Entretanto, o mesmo não ocorre
em se tratando da variância do estimador \(\hat{\theta}_{\pi }\) , que
difere da variância sob o modelo do estimador usual de MV.\medskip
...
as quais são razoavelmente simples de calcular e são consistentes sob
\emph{condições fracas} no plano amostral e na especificação do modelo.
Mesmo quando o estimador pontual de MPV coincide com o estimador usual
de Máxima Ve-ros-si-mi-lhan-ça, a estimativa da variância obtida pelo
procedimento de MPV pode ser preferível aos estimadores usuais da
variância baseados no modelo, que ignoram o plano amostral.

O procedimento MPV fornece estimativas \emph{robustas}, no sentido de
...
é referida à dis-tribuição de aleatorização c\}ão do estimador. Por que
usar a QDPC? A resposta é exatamente para obter maior robustez. Para
entender porque essa abordagem oferece maior robustez, vamos considerar
dois casos.
...
em probabilidade, segundo a distribuição de aleatoriza\c{
...
para a população. \citeonline[p. 81]{Sk89b} discute essa situaç ão,
mos-trando que \(\mathbf{\theta }_{U}\) pode ainda ser um alvo válido
para inferência mesmo quando o modelo
...
\emph{melhor aproximação possível} (em certo sentido) para o verdadeiro
modelo que gera as observações populacionais
(\(f^{*}\left( \mathbf{y};\mathbf{\eta }\right)\), digamos).
Skinner(1989b) reconhece que a \emph{melhor aproximação possível} entre
um conjunto de aproximações ruins ainda seria uma aproximaç ão ruim, e
portanto que a escolha do elenco de modelos especificados pela
distribuição \(f\left( \mathbf{y};\mathbf{\theta }\right)\) deve seguir
os cuidados necessários para garantir que esta escolha forneç a uma
aproximação razoável da realidade.

\textbf{Observação:} Consistência referente à distribuição de
aleatorizaç ão.
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-19}{}{\label{ex:unnamed-chunk-19}}Análise
secundária de tabelas de contingência.(\#Analisec)
\EndKnitrBlock{example}
...
baseados em amostragem Multinomial e Poisson independentes não são mais
válidos. A estatística clássica \(X^{2}\) não tem mais distribuição
\(\chi ^{2}\) e sim uma soma ponderada \$
\sum\emph{\{k\}\delta }\{k\}X\_\{k\}\$ de variáveis \(X_{k}\) IID com
distribuiç ão \(\chi ^{2}\left( 1\right)\). Esse exemplo será
rediscutido com mais detalhes na Seção \ref{raoscott}.
...
e \(\sigma _{e}>0\) são parâmetros desconhecidos do modelo.
...
essa funcionalidade, conforme se discute no Capítulo 10.\medskip 

\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-20}{}{\label{ex:unnamed-chunk-20}}\label{pnad90}Análise
do perfil sócio-econ\^{}\{o\}mico das pessoas ocupadas no setor informal
da economia na área urbana do Rio de Janeiro
\EndKnitrBlock{example}
...
perfil sócio-econ\^{}\{o\}mico das pessoas ocupadas no setor informal da
...
res-pos-ta foi o indicador de pertinência ao setor informal da economia.
...
Exemplo \ref{PNAD}. Como se pode observar dessa descrição, o plano
...
Ve-ros-si-mi-lhan-ça para pesquisas amostrais complexas, tal como foi
feito na derivação de estimadores de MPV a partir de estimadores de
Máxima Verossimilhança. A principal dificuldade é que no contexto de
pesquisas complexas, devido aos pesos distintos das observações e ao
plano amostral utilizado, a função de verossimilhança usual não
representa a distribuição conjunta das observações. Apesar desta
dificuldade ter sido contornada na derivação de estimadores de MPV, a
adaptação fica bem mais difícil no caso de testes da Razão de Máxima
Verossimilhança.
...
ilustrado no Exemplo \ref{exebin} do Capítulo 4. Isto ocorre porque a
...
No Exemplo \ref{exebin} do Capítulo 4 a estatística de teste
...
\begin{table}[tbp] \centering%
%EndExpansion
\caption{Freqüências observadas e proporções hipotéticas }\bigskip \label%
{tab71} 
...
\(p_{j}=N_{j}/N\), onde \(N\) é o tamanho total da populaç\% ão de
...
\(% Z_{p}^{2}=X_{P}^{2}(d)\) conforme esperado. Os valores da
estatística do teste da Razão de Verossimilhança e sua correção de
...
es-ta-tís-ti-ca de bondade de ajuste \(X_{N}^{2}\) de Neyman. No caso de
...
\begin{enumerate}
\item  a metodologia de ajuste de Rao-Scott para as estatísticas de
teste de Pearson e da Razão de Verossimilhança;

\item  e a estatística de Wald baseada no plano amostral.
\end{enumerate}
...
\subsection{Estatística de Pearson com Ajuste de Rao-Scott
(\#raoscott)}\label{estatistica-de-pearson-com-ajuste-de-rao-scott-raoscott}
...
tem distribuição as\$\sin \$toticamente qui-quadrado com
\(\left( J-1\right)\) graus de liberdade se os autovalores forem iguais.
Na prática, esta estatística funciona bem se a variação dos autovalores
estimados for pequena. No cálculo de
\(X_{P}^{2}\left( \hat{\delta}_{.}\right)\) só são necessários os
efeitos multivariados de plano amostral dos \(\hat{p}_{j}\) que aparecem
na diagonal da matriz \(\mathbf{\hat{\Delta}}\). Assim, esta estatística
é adequada em análises secundárias de tabelas de contingência, se forem
divulgadas as estimativas de efeito de plano amostral correspondentes. O
ajuste de primeira ordem de Rao-Scott
\(X_{P}^{2}\left( \hat{\delta}_{.}\right)\) é mais exato do que o ajuste
do EPA médio da estatística \(X_{P}^{2}\left( \hat{d}_{.}\right)\), que
é considerada uma alternativa conservadora de
\(X_{P}^{2}\left( \hat{\delta}_{.}\right)\).
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-21}{}{\label{ex:unnamed-chunk-21}}Teste
...
utilizadas neste exemplo são apresentadas na Tabela \ref
{tab73}.
...
mos-tram que o plano amostral não pode ser ignorado na análise. Queremos
...
Taylor descrito na Seção \ref
{taylor} através da fórmula \eqref{eq:qual24} obtendo

\[
\widehat{\mathbf{V}}_{p}(\widehat{\mathbf{p}})=10^{-6}\times \left[ 
\begin{tabular}{rrrr|r}
$52,27$ & $-3,899$ & $-5,672$ & $-19,29$ & $-23,41$ \\ 
$-3,899$ & $48,16$ & $-29,35$ & $-3,399$ & $-11,52$ \\ 
$-5,672$ & $-29,35$ & $43,80$ & $-8,226$ & $-0,5558$ \\ 
$-19,29$ & $-3,399$ & $-8,226$ & $25,55$ & $5,366$ \\ \hline
...
resultando em \[
\mathbf{P}_{0}/n=10^{-6}\times \left[ 
\begin{tabular}{rrrr|r}
$22,85$ & $-8,855$ & $-7,223$ & $-4,025$ & $-2,745$ \\ 
$-8,855$ & $22,51$ & $-7,051$ & $-3,929$ & $-2,680$ \\ 
$-7,223$ & $-7,051$ & $19,67$ & $-3,205$ & $-2,186$ \\ 
$-4,025$ & $-3,929$ & $-3,205$ & $12,38$ & $-1,218$ \\ \hline
...
estatís-tica de Pearson, dado por \eqref{eq:qual15}, resulta em \[
...
\(% \alpha =5\%\), diferentemente de todos os outros testes\(.\) Os
...
\(% p\)valor do teste. Nesse exemplo, como o número de graus de
liberdade (dado pelo número de unidades primárias na amostra menos o
número de estratos)\(f=m-H=261\) é grande, a correção F tem pouco
efeito, tanto nas estatísticas com ajustes de primeira e segunda ordem
...
X\_\{P\}\^{}\{2\}\left( H\right)\$ calculadas com as estimativas
...
Exemplo \ref{Analisec} do Capítulo 5. Se calcularmos as estatísticas
\(X_{P}^{2}\left( I\right)\) e \(X_{P}^{2}\left( H\right)\) a partir dos
valores dos \(\hat{N}_{lc}\) fornecidos, com a estimativa do tamanho da
popula ção \(\hat{N}\) no lugar de \(n\), os resultados assintóticos
obtidos para amostragem aleatória simples com reposição (IID) deixarão
de ser válidos. Devemos calcular as estatísticas de teste
\(X_{P}^{2}\left( I\right)\) e \(X_{P}^{2}\left( H\right)\) a partir dos
...
X\_\{P\}\^{}\{2\}\left( I\right)\$ e \(X_{P}^{2}\left( H\right)\).
...
\begin{table}[tbp] \centering%
%EndExpansion
\caption{Proporções de linhas em tabela $L\times C$}\bigskip \label{tab82}$
...
X\_\{W\}\^{}\{2\}\left( H\right)\$ para efetuar o teste de homogeneidade
no caso de duas regiões \(\left( L=2\right)\) é dada por
...
\] e tem distribuição de referência \emph{F} com \$\left( C-1\right) \$
...
L\times C\$, onde os dados são extraídos de uma única populaç ão, sem
fixar marginais. Consideremos a Tabela \ref{tab83} com as propor ções
...
da expressão @ref(eq:Tab8, com \(\mathbf{\hat{f}}\) no lugar de
...
de dimensão \$\left( L-1\right) \left( C-1\right) \times \$\linebreak \$
\left( L-1\right) \left( C-1\right)\$ dada por \[
...
\eqref{eq:Tab9} e \eqref{eq:Tab10}, com
...
\] e \(\hat{a}^{2}\) é um estimador do quadrado do coeficiente de variaç
ão dos autovalores desconhecidos de \(\mathbf{\Delta ,}\)
...
\begin{myex}
Correções de EPA médio das estatísticas $X_{P}^{2}\left(
I\right)$ e $X_{P}^{2}\left( H\right)$.
\end{myex}

Considerando os dados do Exemplo \ref{pnad90} do Capítulo 6, vamos
testar a hipótese de independência entre as variáveis Sexo (sx) e
...
variável re tem três níveis: re(1)- Menos de salário mínimo, re(2) - de
1 a 5 salário mínimos e re(3)- mais de 5 salários mínimos. A Tabela
\ref{tab84} apresenta as freqüências nas celas para a amostra
pesquisada.
...
\begin{table}[tbp] \centering%
%EndExpansion
\caption{Freqüências amostrais por celas na PNAD 90}\bigskip \label{tab84} 
\begin{tabular}{|c|cccc|}
\hline\hline
...
freqüências amostrais. Usando o programa Stata, calculamos as
...
utilizado na pesquisa (PNAD 90), conforme descrito no Exemplo \ref
{pnad90} do Capítulo 6.

Vamos considerar o teste de homogeneidade entre as variáveis Sexo e
Renda \textbf{\ }e calcular o efeito de plano amostral médio das
estimativas das proporções nas celas da tabela. A Tabela \ref{tab85}
contém, em cada cela, as estimativas: da proporção na linha, do
desvio-padrão da estimativa da proporção na linha (\$\times
10.000 \$), e do efeito de plano amostral da estimativa de proporção na
linha.
...
variância do estimador da proporção de linha nesta cela é \$ \left(
0,0057269\right) \^{}\{2\}\$. Sob amostragem aleatória simples com
...
1 e 2 da variável sx.

Vamos agora considerar o teste de independência entre as variáveis Sexo
e Renda\textbf{\ }e calcular o efeito de plano amostral médio das
estimativas das proporções nas celas da tabela. A Tabela \ref
{tab86} contém, em cada cela, as estimativas: da proporção na cela, do
desvio-padrão da estimativa da proporção na cela (\$ \times 10.000\$), e
do efeito de plano amostral da estimativa de proporç ão na cela.
...
amostragem aleatória simples com reposiç\% ão, a estimativa da variância
do estimador de proporção na cela é:
...
correspondentes aos níveis 1 e 2 da variável sx.
...
distribui ção de referência \(\chi ^{2}\left( 2\right)\), resultado que
indica rejeição da hipótese de independência entre sx e re, bem como da
hipótese de igualdade de distribuição de renda para os dois sexos a
partir do teste de homogeneidade. O valor comum das estatísticas
\(X_{P}^{2}\left( I\right)\) e \(X_{P}^{2}\left( H\right)\) foi
calculado sem considerar os pesos e o plano amostral. Considerando estes
últimos, mediante a correção de EPA médio das estatísticas clássicas,
obtemos os valores \(X_{P}^{2}\left( I;\hat{d} _{.}\right) =137,117\) e
...
A partir da Tabela \ref{tab86}, examinando as estimativas das proporç
ões nas celas da tabela para cada sexo, observamos uma ordenação
...
são freq"\{u\}entemente considerados na prática para representar
relações entre variáveis, e a inferência é dirigida aos parâmetros
\(\beta _{0}\) e \(\beta _{1}.\)
...
domiciliares são freq"\{u\}entemente usados para ajustar modelos com
efeitos de estratificação, os estratos de análise (pessoas classificadas
por sexo) são formados a posteriori, porque as pessoas da amostra não
são selecionadas em grupos devido à inexistência de cadastros que
...
\section{Modelos Hierárquicos}\label{modelos-hierarquicos}
...
Adotando como referência básica \citeonline[cap.~11]{Sk89a}, vamos
apresentar um resumo de alguns modelos hierárquicos básicos, iniciando
com o caso de variáveis contínuas. Ainda no contexto de estudantes e
turmas do exemplo discutido nesta seção, vamos considerar um modelo
hierárquico de dois níveis com as seguintes variáveis:
...
2 satisfazendo as condições em (\ref{9.5}). As equações (\ref
{9.9}) e (\ref{9.10}) definem um modelo hierárquico, que pode ser
...
\(\eta _{2j}\) (de nível 2), torna (\ref{9.11}) um modelo misto. Se os
erros fossem suprimidos em (\ref{9.10}), o modelo especificado só teria
efeitos fixos e a estimação dos parâmetros não traria qualquer problema.
Entretanto, a exclusão dos erros de nível 2 em (\% \ref{9.10}) não seria
razoável, pois as variáveis definidas no nível 2 não determinam
completamente os coeficientes dentro das turmas. Este aspecto
...
Além da parte fixa, o modelo contém uma \emph{parte residual, }com os
...
\(\varepsilon _{ij},\) que é o desvio do \(i\)\% -ésimo aluno com
relação à média da turma \(j\). Vamos supor, ainda, que os
\(\eta _{0j}\) e os \(\varepsilon _{ij}\) são independentes entre si e
os \(\eta _{0j}\) são independentes dos \(% \varepsilon _{ij}\) , com
...
A parte aleatória do modelo (\ref{9.12}) é o termo
\(\eta _{0j}+\varepsilon _{ij}\), com distribuição tendo parâmetros
\(% \sigma _{0}^{2}\) e \(\sigma ^{2}\) a serem estimados.

O modelo (\ref{9.12}) não permite estudar interações entre variáveis nos
dois níveis hierárquicos. Para isto teríamos de supor, por exemplo, que
a diferença de desempenho entre sexos varia com as turmas, requerendo
que o modelo fosse alterado, fazendo \(\beta _{1j}\) depender das
variáveis \(CST_{j}\) e \(EXP_{j}\). Isto introduziria mais erros
aleatórios no modelo e mais parâmetros a serem estimados.

Voltando ao modelo básico de dois níveis hierárquicos (\ref{9.11}\% ),
...
\begin{equation}
CORR_{M}\left( ESC_{ij},ESC_{ij^{^{\prime }}}\right) =\frac{\sigma _{0}^{2}}{%
\sigma _{0}^{2}+\sigma ^{2}}.  \label{eq:hier15}
\end{equation}

A expressão (\ref{9.15}) define a \emph{correlação intraclasse }\% usual
que, em nosso exemplo, mede o grau de similaridade entre alunos dentro
das turmas ou o grau de conglomeração da variável resposta
\(% ESC_{ij}\) por turmas. Ela é a fração da variância residual
atribuída à variância intra-conglomerado. No caso de se supor
adicionalmente a normalidade dos dados, foram propostos vários métodos
para estimação dos parâmetros do modelo (\% \ref{9.11}), entre os quais
os métodos de Máxima Verossimilhanç\% a, de Máxima Verossimilhança
Restrita e o Método Iterativo de Mínimos Quadrados Generalizados.
Detalhes destes métodos de estima\c{%
c}ão não serão abordados neste texto, e o leitor interessado deve
consultar, por exemplo, \citeonline{bryk}.
...
definido por \citeonline{Rubin76}. Este raciocínio, porém, não é
aplicável quando unidades em qualquer nível da hierarquia são
selecionadas com probabilidades desiguais, de formas não consideradas
pelas covariáveis. Por exemplo, quando as unidades são selecionadas com
probabilidades proporcionais a uma medida de tamanho que é relacionada à
variável resposta. \citeonline{Pfefetalli} apresentam uma forma de
incorporar pesos no ajuste de modelos hierárquicos para compensar
diferentes probabilidades de inclusão das unidades na amostra.

\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-22}{}{\label{ex:unnamed-chunk-22}}Plano
amostral de pesquisa educacional (\citeauthoronline{lethonen},
\citeyear{lethonen}, p.~297).
...
chamaremos aqui de \emph{análise ingênua}. Neste caso, os coeficientes
podem ser estimados pelo método padrão de Máxima Verossimilhança,
utilizando um dos pacotes padrões. Os resultados dessa análise são
apresentados na Tabela \ref{tab91}.
...
Outra opção é a \emph{análise agregada}, que incorpora o plano amostral
e os pesos através do método de MPV para estimar parâmetros, e do uso de
estatísticas baseadas no plano amostral para testar hipóteses. Esta
abordagem pode ser usada também na etapa de seleção de modelos, com
testes de siginificância baseados, por exemplo, na estatística de Wald
ou ajustes desta, no caso de instabilidade. Os resultados dessa análise
são apresentados na Tabela \ref{tab92}.
...
\multicolumn{1}{|r|}{$5,02$} & \multicolumn{1}{|r|}{$0,00$} & 
...
A coluna de \(p\)valores da Tabela \ref{tab92} indica que o coeficiente
de \(% TEMP\) é significantemente diferente de zero, e também que a
hipótese de nulidade do coeficiente de \(EXP\) não é rejeitada no nível
de significância \(\alpha =5\%\).
...
mesmos que os da análise reportada na Tabela \ref{tab92}, conforme se
...
Finalmente, a \emph{análise desagregada} que, neste exemplo, usaria um
\emph{modelo hierárquico} com dois níveis, a saber:
...
No modelo hierárquico, para cada nível se considera uma variaç\% ão
...
modificar o modelo (\ref{9.16}) para incluir o efeito da turma
empregando

\begin{equation}
\log \left( \frac{p_{jk}}{1-p_{jk}}\right) =\beta _{0}+\beta
_{1}TEMP_{jk}+\beta _{2}EXP_{j}+u_{k}.  \label{eq:hier17}
\end{equation}

O erro aleatório \$u\_\{k\}\sim N\left( 0,\sigma \_\{u\}\^{}\{2\}\right)
\$ representa a variação aleatória no nível 2. A variaç\% ão no nível 1,
entre alunos, é introduzida da seguinte forma. Sob a hipótese binomial,
em cada domínio, a proporção de alunos atingindo o nível adequado tem
...
\(% \sigma _{u}^{2}=0,42\) com desvio-padrão \(0,189\), e portanto é
...
\BeginKnitrBlock{example}
\protect\hypertarget{ex:unnamed-chunk-23}{}{\label{ex:unnamed-chunk-23}}(\citeauthoronline{bryk},
\citeyear{bryk}, Cap. 5)
\EndKnitrBlock{example}
...
\(x_{2j}=\) Status Sócio-Econ\^{}\{o\}mico Médio;
...
\(x_{6j}=\) Mistura de Status Sócio-Econ\^{}\{o\}mico;
...
\] \[
\mbox{\emph{Correlação Intra-Escola}}=\frac{\hat{\tau}_{00}}{\hat{%
...
escola) explicada pelas variáveis \$\left(
x\_\{1\},\ldots ,x\_\{6\}\right) \$ aumentou para \[
\frac{0,084-0,055}{0,084}=35.
\]

Embora esse aumento do poder explicativo do modelo já tenha sido
subs-tan-cial, ainda é relativamente baixa a proporção de varia\% ção
...
variáveis \$\left( x\_\{1\},\ldots ,x\_\{7\}\right) \$, definidas no
nível 2, para explicar a variação da eficiência média do professor por
escola\textbf{.}
...
variáveis \$\left( x\_\{1\},\ldots ,x\_\{7\}\right) \$ aumentou para \[
...
que essa variável\emph{\ }é importante para explicar a variação na
eficácia do professor. O Modelo III já atinge um nível razoável de poder
explicativo e poderia ser considerado satisfatório para algumas
finalidades.
...
do professor \(Y_{ij}\) dependendo das características \$\left(
x\_\{1j\},\ldots ,x\_\{7j\}\right) \$ da escola, que teriam seus valores
``repetidos'' para os professores de uma mesma escola. Esse modelo pode
ser escrito como \[
...
Ponderados de \$\left(
\gamma \emph{\{0\},\gamma }\{1\},\ldots \gamma \_\{7\}\right) \$ com
pesos dados por \(n_{j}=\) número de professores da escola \(j\).
...
A introdução da variável \(Comunit\)\emph{, }neste modelo, só aumentou a
quantidade de variação explicada em \(2,5\%\). A julgar por este
resultado, a importância da variável \(Comunit\)\emph{\ }é pequena. Este
resultado é enganador devido ao fato de usar, no cálculo da razão de
variação explicada, a variação total \$\left(
\tau \_\{00\}+\sigma \^{}\{2\}\right) \$ no denominador. No modelo
hierárquico correspondente (Modelo III) este denominador é
\(\tau _{00},\) que é a parte explicável da variação. A estatística de
...
aleatórios. Tais modelos poderiam ser generalizados mediante suposiç\%
ão de que os coeficientes de regressão nas variáveis preditoras
...
\allowbreak pes-qui-sas complexas, indicando em cada caso como a análise
desagregada poderia ajudar a solucionar o problema. Esta parte é um
resumo da seção introdutória do Capítulo 5 do livro de
\citeonline{bryk}, denominada `'Pontos básicos sobre efeitos
organizacionais da pesquisa''.\medskip
...
\emph{indivíduo}) e no nível 2 (\emph{organização}).\medskip
...
amostral), que decorre da amostragem por conglomerado.\medskip
...
fen\^{}\{o\}meno seja, freq"\{u\}entemente, considerado como de
distúrbio do ponto de vista metodológico, as causas da heterogeneidade
da regressão são muitas vezes de interesse substantivo. Modelos
hierárquicos possibilitam ao pesquisador estimar um conjunto de
coeficientes de regressão para cada unidade organizacional e, então,
modelar a variação de conjuntos de coeficientes entre organizações como
resultados multivariados a serem explicados por fatores
organizacionais.\medskip
...
implicações para a modelagem desagregada.\medskip
...
pacotes padrões até recentemente, e freq"\{u\}entemente requerem o
emprego de pacotes ou procedimentos especializados para seu ajuste e
análise.

Quando a modelagem for feita com a finalidade de incorporar aspectos do
planejamento amostral, tais como estratificação, conglomeraç\% ão e
probabilidades desiguais de inclusão, a situação desejável é incorporar
na formulação do modelo as informa\c{c%
}ões necessárias para que o plano amostral seja ignorável na etapa de
estimação dos parâmetros. Mesmo quando se pode incorporar no modelo as
informações sobre a estrutura populacional, há casos nos quais o plano
amostral é não ignorável e pesos precisam ser incorporados para ajustar
o modelo (veja \citeauthoronline{Pfefetalli}, \citeyear{Pfefetalli}.
...
desafios não triviais ao analista: a especificaç\% ão detalhada da forma
do modelo, a estimação de seus inúmeros parâmetros dada apenas uma
amostra das unidades da popula\c{%
c}ão, a interpretação das estimativas dos parâmetros e o diagnóstico do
ajuste efetuado podem todas se tornar tarefas bastante complexas. Essa
dificuldade é também mencionada por \citeonline[p.~9]{Sk89a}.
...
nos quais, entretanto, as informações de identifica\c{c%
}ão do plano amostral são omitidas (total ou parcialmente) para evitar a
revelação indesejada de informações individuais `'sensitivas''.\medskip
...
internet: www.rti.org/patents/sudaan.html\medskip
